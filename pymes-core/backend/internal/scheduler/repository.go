...
func (r *Repository) ListAutoFetchRateOrgs(ctx context.Context) ([]uuid.UUID, error) {
	var ids []uuid.UUID
	err := r.db.WithContext(ctx).Table("tenant_settings").Select("tenant_id").Where("auto_fetch_rates = true").Scan(&ids).Error
	return ids, err
}

func (r *Repository) UpsertExchangeRate(ctx context.Context, tenantID uuid.UUID, fromCurrency, toCurrency, rateType string, buyRate, sellRate float64, source string, rateDate time.Time) error {
	return r.db.WithContext(ctx).Exec(`
		INSERT INTO exchange_rates (id, tenant_id, from_currency, to_currency, rate_type, buy_rate, sell_rate, source, rate_date, created_at)
		VALUES (gen_random_uuid(), ?, ?, ?, ?, ?, ?, ?, ?, now())
		ON CONFLICT (tenant_id, from_currency, to_currency, rate_type, rate_date)
		DO UPDATE SET buy_rate = EXCLUDED.buy_rate, sell_rate = EXCLUDED.sell_rate, source = EXCLUDED.source, created_at = now()
	`, tenantID, fromCurrency, toCurrency, rateType, buyRate, sellRate, source, time.Date(rateDate.Year(), rateDate.Month(), rateDate.Day(), 0, 0, 0, 0, time.UTC)).Error
}

func (r *Repository) ListDueRecurring(ctx context.Context, day time.Time) ([]RecurringDue, error) {
	var rows []RecurringDue
	err := r.db.WithContext(ctx).Raw(`
		SELECT id, tenant_id, description, amount, currency, category, payment_method, frequency, day_of_month, next_due_date
...
func (r *Repository) ApplyRecurringExpense(ctx context.Context, item RecurringDue, paidAt, nextDue time.Time) error {
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		if err := tx.Exec(`
			INSERT INTO cash_movements (id, tenant_id, type, amount, currency, category, description, payment_method, reference_type, reference_id, created_by, created_at)
			VALUES (gen_random_uuid(), ?, 'expense', ?, ?, ?, ?, ?, 'recurring_expense', ?, 'scheduler', now())
		`, item.TenantID, item.Amount, item.Currency, item.Category, item.Description, item.PaymentMethod, item.ID).Error; err != nil {
			return err
		}
		return tx.Exec(`
...
func (r *Repository) ListDueSchedulingReminders(ctx context.Context, now time.Time, limit int) ([]SchedulingReminderDue, error) {
	...
	err := r.db.WithContext(ctx).Raw(`
		SELECT
			b.tenant_id,
			COALESCE(o.slug, b.tenant_id::text) AS tenant_slug,
			b.id AS booking_id,
			b.customer_name,
			b.customer_email,
		...
		JOIN tenants o ON o.id = b.tenant_id
		JOIN tenant_settings ts ON ts.tenant_id = b.tenant_id
