...
import { getIntakes, createIntake, submitIntake, updateIntake } from '../lib/professionalsApi';
import type { Intake } from '../lib/professionalsTypes';
  ...
  return (
    <>
      ...
      <div className="card">
        <div className="card-header">
          <h2>Listado</h2>
          <span className="badge badge-neutral">{filtered.length} ingresos</span>
        </div>
        {loading ? (
          <div className="spinner" />
        ) : filtered.length === 0 ? (
          <div className="empty-state">
            <p>No hay ingresos{filterStatus ? ` con estado "${filterStatus}"` : ''}</p>
          </div>
        ) : (
