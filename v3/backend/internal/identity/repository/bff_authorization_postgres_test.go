...
import (
	...
	commercedomain "github.com/devpablocristo/pymes/v3/backend/internal/commerce/domain"
...
type authorizationCommerceStub struct {
	commercehandler.Commerce
	calls int
}

func (s *authorizationCommerceStub) capture(ctx context.Context) {
	principal, ok := identityusecases.PrincipalFromContext(ctx)
...
func TestPostgresPrincipalDrivesBFFMutationAuthorization(t *testing.T) {
	...
	memberships := identityrepository.New(pool)
	for _, test := range tests {
		for _, endpoint := range mutationEndpoints(test.localOrg) {
			commerce := &authorizationCommerceStub{}
			auth := identityaccess.ClerkAuthenticator{
			...
			commercehandler.NewHTTPServer(commerce, auth).Handler().ServeHTTP(recorder, request)
			...
			if commerce.calls != wantCalls {
				t.Fatalf("case=%s endpoint=%s calls=%d want=%d", test.name, endpoint.name, commerce.calls, wantCalls)
			}
		}
	}
}
...
func TestPostgresOwnerAndAdminPersistEveryBFFMutation(t *testing.T) {
	...
	for _, role := range []string{"owner", "admin"} {
		...
		commands := commerceusecases.Commands{Store: store, Now: store.Clock}
		auth := identityaccess.ClerkAuthenticator{
			Memberships: identityrepository.New(pool),
			Verifier: fixedSessionVerifier{claims: clerk.SessionClaims{
		...
		for _, endpoint := range endpoints {
			...
			request.Header.Set("X-Source-Version", "1")
			recorder := httptest.NewRecorder()
			commercehandler.NewHTTPServer(commands, auth).Handler().ServeHTTP(recorder, request)
