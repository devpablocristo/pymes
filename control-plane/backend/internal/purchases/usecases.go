...
type Usecases struct { repo RepositoryPort; audit AuditPort }
...
func (u *Usecases) Create(ctx context.Context, in CreateInput) (purchasesdomain.Purchase, error) {
	...
	if err != nil { return purchasesdomain.Purchase{}, err }
	if u.audit != nil { u.audit.Log(ctx, in.OrgID.String(), in.CreatedBy, "purchase.created", "purchase", out.ID.String(), map[string]any{"number": out.Number, "total": out.Total}) }
	return out, nil
}
...
func (u *Usecases) GetByID(ctx context.Context, orgID, id uuid.UUID) (purchasesdomain.Purchase, error) {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) { return purchasesdomain.Purchase{}, apperror.NewNotFound("purchase", id.String()) }
...
func (u *Usecases) Update(ctx context.Context, in UpdateInput, actor string) (purchasesdomain.Purchase, error) {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) { return purchasesdomain.Purchase{}, apperror.NewNotFound("purchase", in.ID.String()) }
		return purchasesdomain.Purchase{}, err
	}
	...
	if err != nil { return purchasesdomain.Purchase{}, err }
	if u.audit != nil { u.audit.Log(ctx, in.OrgID.String(), actor, "purchase.updated", "purchase", out.ID.String(), map[string]any{"status": out.Status, "total": out.Total}) }
	return out, nil
}

func (u *Usecases) prepareCreate(ctx context.Context, in CreateInput) (CreateInput, error) {
	if in.OrgID == uuid.Nil { return CreateInput{}, apperror.NewBadInput("org_id is required") }
	if len(in.Items) == 0 { return CreateInput{}, apperror.NewBadInput("items are required") }
	if in.SupplierID != nil && *in.SupplierID != uuid.Nil && strings.TrimSpace(in.SupplierName) == "" {
		if name, err := u.repo.GetSupplierName(ctx, in.OrgID, *in.SupplierID); err == nil && strings.TrimSpace(name) != "" { in.SupplierName = name }
	}
	if strings.TrimSpace(in.SupplierName) == "" { in.SupplierName = "Proveedor sin nombre" }
	in.Status = defaultString(strings.ToLower(in.Status), "draft")
	if in.Status != "draft" && in.Status != "received" && in.Status != "partial" && in.Status != "voided" { return CreateInput{}, apperror.NewBadInput("invalid status") }
	...
	for idx, item := range in.Items {
		if item.Quantity <= 0 || item.UnitCost < 0 { return CreateInput{}, apperror.NewBadInput("invalid purchase item") }
		taxRate := item.TaxRate
		if taxRate <= 0 { taxRate = defaultTax }
...
func defaultString(v, def string) string { if strings.TrimSpace(v) == "" { return def }; return strings.TrimSpace(v) }

func markReceivedAt(status string) *time.Time {
	if status == "received" { now := time.Now().UTC(); return &now }
	return nil
}
