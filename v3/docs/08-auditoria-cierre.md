...
| Controles GitHub | parcial, bloqueo fail-closed vigente | `main` exige exactamente `Pymes V3 validate`, no exige reviewers y conserva historial lineal, resolución de conversaciones y enforcement administrativo; STG está limitado a `main`; PRD tiene reviewers sólo para desplegar y evita self-review. Falta desactivar en la UI el bypass administrativo de PRD y cargar en STG/PRD un token de auditoría fine-grained con `Administration:read` |
