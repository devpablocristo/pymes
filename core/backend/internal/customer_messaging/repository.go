...
import (
	...
	whmodels "github.com/devpablocristo/pymes/core/backend/internal/customer_messaging/repository/models"
)
...
func (r *Repository) GetConnectionByPhoneNumberID(ctx context.Context, phoneNumberID string) (Connection, error) {
	var m whmodels.WhatsAppConnection
	err := r.db.WithContext(ctx).Where("phone_number_id = ? AND is_active = true", strings.TrimSpace(phoneNumberID)).Take(&m).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
...
func (r *Repository) UpdateMessageStatus(ctx context.Context, waMessageID string, status domain.MessageStatus, errorCode, errorMsg string) error {
	...
	result := r.db.WithContext(ctx).Model(&whmodels.WhatsAppMessage{}).Where("wa_message_id = ?", strings.TrimSpace(waMessageID)).Updates(updates)
	if result.Error != nil {
		return result.Error
	}
...
func (r *Repository) ListMessages(ctx context.Context, filter domain.MessageFilter) ([]domain.Message, int, error) {
	...
	var rows []whmodels.WhatsAppMessage
	if err := q.Order("created_at DESC").Offset(filter.Offset).Limit(limit).Find(&rows).Error; err != nil {
		return nil, 0, err
	}
...
func (r *Repository) UpdateRecipientStatus(ctx context.Context, recipientID uuid.UUID, status domain.RecipientStatus, waMessageID, errorMsg string) error {
	...
	return r.db.WithContext(ctx).Model(&whmodels.WhatsAppCampaignRecipient{}).Where("id = ?", recipientID).Updates(updates).Error
}

func (r *Repository) ListCampaignRecipients(ctx context.Context, campaignID uuid.UUID) ([]domain.CampaignRecipient, error) {
	var rows []whmodels.WhatsAppCampaignRecipient
...
func (r *Repository) ListConversations(ctx context.Context, orgID uuid.UUID, assignedTo, status string, limit int) ([]domain.Conversation, error) {
	...
	if err := q.Order("last_message_at DESC NULLS LAST").Limit(limit).Find(&rows).Error; err != nil {
		return nil, err
	}
...
func (r *Repository) UpdateConversationLastMessage(ctx context.Context, conversationID uuid.UUID, preview string, inbound bool) error {
	now := time.Now()
	updates := map[string]any{"last_message_at": now, "last_message_preview": preview, "updated_at": now}
	if inbound {
