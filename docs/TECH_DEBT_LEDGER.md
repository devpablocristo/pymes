...
| DEBT-004 | AI | removed | `internal_chat` dependia de piezas del runtime historico `pymes/ai`. | El chat interno heredaba acoplamiento y fallback histórico. | Runtime movido a Axis Companion; Pymes conserva solo gateway de capabilities y alias legacy de env. |
| DEBT-005 | AI | removed | El runtime historico concentraba routing, tools, prompts, persistencia y fallback. | Alto costo de cambio y riesgo de respuestas no confiables. | Checks/runtime de IA viven en Axis Companion; Pymes no ejecuta LLM local. |
