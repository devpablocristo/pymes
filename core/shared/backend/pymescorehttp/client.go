// Package pymescorehttp es el cliente HTTP compartido para llamar al API interno de core desde lambdas verticales.
...
func (c *Client) decode(status int, body []byte) (map[string]any, error) {
	if status >= 400 {
		return nil, fmt.Errorf("core returned %d: %s", status, string(body))
	}
