  ...
  return (
    <>
      <div className="cry__stats">
        <div className="cry__stat-card">
          <div className="cry__stat-label">Valor total</div>
          <div className="cry__stat-value">{fmtUsd(total)}</div>
        </div>
        <div className="cry__stat-card">
          <div className="cry__stat-label">Activos</div>
          <div className="cry__stat-value">{coins.length}</div>
        </div>
        <div className="cry__stat-card">
          <div className="cry__stat-label">Mejor hoy</div>
          <div className="cry__stat-value cry__change--up">+{Math.max(...coins.map((c) => c.change)).toFixed(1)}%</div>
        </div>
      </div>
      <div className="card">
        <table className="cry__table">
          ...
          <tbody>
            {coins.map((c) => (
              <tr key={c.id}>
                <td>
                  <div className="cry__coin">
                    <div className="cry__coin-icon" style={{ '--cry-coin-bg': c.color } as CSSProperties}>
                      {c.icon}
                    </div>
                    <div>
                      <strong>{c.name}</strong>
                      <br />
                      <span className="cry__coin-symbol">{c.symbol}</span>
                    </div>
                  </div>
                </td>
                ...
                <td className={c.change >= 0 ? 'cry__change--up' : 'cry__change--down'}>
                  {c.change >= 0 ? '+' : ''}
                  {c.change}%
                </td>
                <td>{c.allocation}%</td>
        ...
        <tbody>
          {coins.map((c) => (
            <tr key={c.id}>
              ...
              <td>{c.marketCap}</td>
              <td>
                <Sparkline data={c.spark} color={c.change >= 0 ? 'var(--color-success)' : 'var(--color-danger)'} />
              </td>
              <td>
                <button
                  type="button"
                  className={`cry__watch-btn${watched.has(c.id) ? ' cry__watch-btn--active' : ''}`}
                  onClick={() => toggleWatch(c.id)}
                >
                  <IconStar filled={watched.has(c.id)} />
                </button>
              </td>
            </tr>
          ))}
...
function PortfolioView({ coins }: { coins: Coin[] }) {
  ...
  return (
    <>
      <div className="cry__stats">
        <div className="cry__stat-card">
          <div className="cry__stat-label">Portfolio</div>
          <div className="cry__stat-value">{fmtUsd(total)}</div>
        </div>
        <div className="cry__stat-card">
          <div className="cry__stat-label">Ganancia 24h</div>
          <div className="cry__stat-value cry__change--up">+$842</div>
        </div>
      </div>
      <div className="card">
        <table className="cry__table">
          ...
          <tbody>
            {coins.map((c) => (
              <tr key={c.id}>
                ...
                <td>
                  {c.holding.toLocaleString()} {c.symbol}
                </td>
                <td>{fmtUsd(c.price)}</td>
                <td className={c.change >= 0 ? 'cry__change--up' : 'cry__change--down'}>
                  {c.change >= 0 ? '+' : ''}
...
export function CryptoPage() {
  ...
  return (
    <PageLayout className="cry" title="Crypto (demo)" lead="Billetera, mercado y asignación — datos ilustrativos.">
      <div className="cry__tabs">
        {(
          ...
          <button
            key={id}
            type="button"
            className={`cry__tab ${tab === id ? 'cry__tab--active' : ''}`}
            onClick={() => setTab(id)}
          >
            {label}
          </button>
