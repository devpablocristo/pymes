...
func (f *fakeRepo) List(ctx context.Context, p ListParams) ([]Vehicle, int64, bool, *uuid.UUID, error) {
	_ = ctx
	_ = p
	return nil, 0, false, nil, nil
}

func (f *fakeRepo) Create(ctx context.Context, in Vehicle) (Vehicle, error) {
	_ = ctx
	f.created = in
	in.ID = uuid.New()
	return in, nil
...
func (f *fakeRepo) GetByID(ctx context.Context, orgID, id uuid.UUID) (Vehicle, error) {
	_ = ctx
	_ = orgID
	_ = id
...
func (f *fakeRepo) Update(ctx context.Context, in Vehicle) (Vehicle, error) {
	_ = ctx
	return in, nil
}
...
func TestCreateAutofillsCustomerNameFromControlPlane(t *testing.T) {
	...
	out, err := uc.Create(context.Background(), Vehicle{
		OrgID:        orgID,
...
func TestCreateRejectsInvalidCustomerReference(t *testing.T) {
	...
	_, err := uc.Create(context.Background(), Vehicle{
		OrgID:        orgID,
		CustomerID:   &customerID,
		LicensePlate: "AB123CD",
		Make:         "Ford",
