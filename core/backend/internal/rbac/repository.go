...
import (
	...
	"github.com/devpablocristo/pymes/core/backend/internal/rbac/repository/models"
	rbacdomain "github.com/devpablocristo/pymes/core/backend/internal/rbac/usecases/domain"
)
...
func (r *Repository) CreateRole(ctx context.Context, in rbacdomain.Role) (rbacdomain.Role, error) {
	now := time.Now().UTC()
	m := models.RoleModel{
		ID:          uuid.New(),
		OrgID:    in.OrgID,
	...
	if err := r.replaceRolePermissions(ctx, m.ID, in.Permissions); err != nil {
		return rbacdomain.Role{}, err
	}
	perms, err := r.loadRolePermissions(ctx, m.ID)
	if err != nil {
		return rbacdomain.Role{}, err
	}
...
func (r *Repository) UpdateRole(ctx context.Context, in rbacdomain.Role) (rbacdomain.Role, error) {
	now := time.Now().UTC()
...
func (r *Repository) loadRolePermissions(ctx context.Context, roleID uuid.UUID) ([]rbacdomain.Permission, error) {
	var rows []models.RolePermissionModel
	if err := r.db.WithContext(ctx).Where("role_id = ?", roleID).Order("resource, action").Find(&rows).Error; err != nil {
		return nil, err
	}
...
func (r *Repository) replaceRolePermissions(ctx context.Context, roleID uuid.UUID, perms []rbacdomain.Permission) error {
	if err := r.db.WithContext(ctx).Where("role_id = ?", roleID).Delete(&models.RolePermissionModel{}).Error; err != nil {
		return err
	}
	...
	for _, p := range perms {
		...
		seen[key] = struct{}{}
		if err := r.db.WithContext(ctx).Create(&models.RolePermissionModel{
			ID:       uuid.New(),
			RoleID:   roleID,
			Resource: resource,
			Action:   action,
