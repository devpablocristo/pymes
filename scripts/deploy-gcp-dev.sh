...
GOVERNANCE_URL_EFFECTIVE="${GOVERNANCE_URL:-${REVIEW_URL_VALUE}}"
GOVERNANCE_API_KEY_EFFECTIVE="${GOVERNANCE_API_KEY:-${REVIEW_API_KEY_VALUE}}"
...
upsert_secret REVIEW_API_KEY "$GOVERNANCE_API_KEY_EFFECTIVE"
...
if [[ -n "$GOVERNANCE_API_KEY_EFFECTIVE" ]]; then
  BACKEND_SECRET_VARS="${BACKEND_SECRET_VARS},GOVERNANCE_API_KEY=REVIEW_API_KEY:latest"
fi
...
if gcloud secrets describe REVIEW_API_KEY --project="$PROJECT_ID" >/dev/null 2>&1; then
  AI_SECRET_VARS="${AI_SECRET_VARS},GOVERNANCE_API_KEY=REVIEW_API_KEY:latest"
fi
