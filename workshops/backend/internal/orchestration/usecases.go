...
type workOrderPort interface {
	GetByID(ctx context.Context, orgID, id uuid.UUID) (workorders.WorkOrder, error)
	SaveIntegrations(ctx context.Context, orgID, id uuid.UUID, quoteID, saleID *uuid.UUID, status *string) (workorders.WorkOrder, error)
}
...
func (u *Usecases) CreateQuoteFromWorkOrder(ctx context.Context, orgID string, workOrderID uuid.UUID, actor string) (map[string]any, error) {
	...
	payload := map[string]any{
		"org_id":        orgID,
		"customer_name": fallback(order.CustomerName, order.VehiclePlate),
		"notes":         order.Notes,
...
func toCommercialItems(items []workorders.WorkOrderItem) []map[string]any {
