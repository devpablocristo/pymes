...
1. **El frontend tiene una reinvención casera de FSM** (`buildFullyConnectedStatusStateMachine` en `ui/src/modules/billing/billingHelpers.ts`) usada por 4 dominios (sales, quotes, purchases, invoices), conectada por su propio "fully-connected" (todo pasa con todo). Esto es la causa raíz del "drag arbitrario funciona" — el FE deja mover a cualquier columna y el BE simplemente no validaba transiciones. Solucionar el BE sin tocar el FE deja el bug visible: el usuario arrastra y recibe 409 en vez de un drag bloqueado pre-server. **El frontend ya importa `@devpablocristo/core-fsm`** (en `ui/src/modules/crud/kanbanTransitionModel.ts`) pero esa primitiva está infrautilizada — solo el módulo genérico de kanban la usa, no los dominios.
2. **`MapFSMError` y `RegisterStatusEndpoint` ambos en `internal/shared/handlers/` invierten la arquitectura**: si `sales/usecases.go` importa `internal/shared/handlers` para llamar `MapFSMError`, la capa de aplicación pasa a depender de la capa HTTP/handler. **Corrección obligatoria**: separar `MapFSMError` (agnóstico de HTTP, depende solo de `core/concurrency/go/fsm` + `core/errors/go/domainerr`) en `core/backend/internal/shared/status/fsm_errors.go`. `RegisterStatusEndpoint` queda en `handlers/` porque sí depende de Gin/RBAC.
3. **Audit/timeline/webhook post-transición** están bien extraídos como ports unificados (24 dominios usan la misma interface), pero la **decisión "emit timeline solo si hay party_id"** está dispersa — sales emite si hay `customer_id`, purchases si hay `supplier_id`, quotes no emite. Si se quiere normalizar, va como helper local en core (no es agnóstico — depende de la noción de "party" del producto).
...
├── pymes/                   # producto
│   ├── core/backend/  # Go (4 verticales agregan más)
│   ├── ui/            # consola React
│   ├── ai/                  # FastAPI
...
| **core** | `core/http/gin/go` | `Respond(c, err)` mapea domainerr→HTTP | OK. Ya usado vía `core/shared/backend/httperrors` | `respond.go` |
| **core** | `core/http/go/pagination` | `Config`, `NormalizeLimit` | OK. Usado por `internal/shared/handlers/pagination.go` | `pagination.go` |
| **modules** | `modules/crud/paths/go` | `SegmentArchived`, `SegmentArchive`, `SegmentRestore`, `SegmentHard` | Solo lo usa `quotes`. Sales/purchases/invoices usan paths raw. | `core/backend/internal/quotes/handler.go:55` |
...
| **modules** | `modules/crud/ui/ts` | `CrudPage`, `CrudShellHeaderActionsColumn`, ~22 imports en frontend | OK. Frontend lo usa. | `ui/src/modules/crud/CrudResourceShellHeader.tsx` |
...
| **core** | `internal/sales/`, `quotes/`, `purchases/` | UpdateStatus heterogéneo | sales/quotes no validan transiciones; purchases usa switch hardcoded | `sales/usecases.go:362-391`, `quotes/usecases.go:137-176`, `purchases/usecases.go:181-217` |
| **core** | `internal/invoices/` | Sin endpoint /status; status entra por PATCH genérico | Sin validación, sin audit `invoice.status_updated` | `invoices/handler.go:43`, `usecases.go:184` |
| **core** | `internal/shared/handlers/` | RBAC, pagination wrappers, auth ctx | Lugar correcto para `RegisterStatusEndpoint` | `pagination.go`, `rbac_middleware.go` |
...
4. **¿Depende de RBAC interno, Gin handler, audit names específicos, DTOs concretos, o cualquier decisión de negocio del producto?** Sí → vive en `core` (o un vertical).
5. **¿Es reusable pero hoy mezcla parte agnóstica + parte específica?** Separar: lo agnóstico va a `core`/`modules`, lo específico queda como adapter en `core`.
...
| `modules/*` | `core/*` | `pymes/*` |
| `core/backend/internal/*` | `core/*`, `modules/*`, `core/shared/*`, otros internal hermanos vía interfaces | NUNCA estructuras GORM/handlers de otro internal |
| `core/shared/backend/*` | `core/*`, `modules/*` | `core/backend/internal/*` (back-dep) |
| `verticales (workshops/, professionals/, ...)` | core, modules, core/backend HTTP API | dominio interno de otros verticales |
| `ui/src/*` | `@devpablocristo/core-*`, `@devpablocristo/modules-*` | librerías privadas de otros productos |
...
rg "core/backend/internal" /home/pablocristo/Proyectos/pablo/pymes/core/shared/  # debe estar VACÍO (back-dep)
```
...
grep -E "\"@devpablocristo/.*\":\s*\"(file:|link:)" /home/pablocristo/Proyectos/pablo/pymes/ui/package.json
# debe estar VACÍO
```
...
- **Grafos concretos** de cada dominio (sales, quotes, etc.): viven en el dominio (`core/backend/internal/<domain>/fsm.go`). Nunca subir grafos pymes a core/modules.
- **Mapeo FSM → HTTP error**: helper local en core (`RegisterStatusEndpoint` y `MapFSMError`). Justificación: el mensaje de error tiene que poder ser internacionalizado/customizado por producto.
...
|---|---|---|---|---|---|
| Validación de status (BE) | sales, quotes, purchases | sales/quotes hardcoded set; purchases con switch + transiciones | Bug latente: drag a estado inválido lo guarda | `core/backend/internal/<dom>/fsm.go` con `core/concurrency/go/fsm` | **Migrar a FSM** (Sección 6) |
| Falta UpdateStatus (BE) | invoices | Status entra por PATCH /:id genérico, sin audit `invoice.status_updated` | No auditable, divergencia con sales/quotes/purchases | `core/backend/internal/invoices/` | **Agregar endpoint** + FSM |
| Helper register de endpoint /status | sales, quotes, purchases | ~40 líneas idénticas (parsing ID + body + mapeo error) | Drift accidental al copiar | `core/backend/internal/shared/handlers/status_endpoint.go` | **Crear `RegisterStatusEndpoint[T]`** (Sección 5.3) |
| FSM "fully-connected" en FE | sales, quotes, purchases, invoices | `buildFullyConnectedStatusStateMachine` permite todo→todo | Drag arbitrario (causa raíz UX bug) | `ui/src/modules/billing/billingHelpers.ts` | **Reemplazar con `core-fsm` Builder** que espeje el BE |
| Endpoint paths hardcodeados (FE) | 4 configs | `/v1/sales/${id}/status`, etc. raw strings | Drift al refactorizar | Constantes locales (no merece módulo nuevo) | **Centralizar en `ui/src/lib/endpoints.ts`** (PR auxiliar) |
| Status labels (i18n) | invoices vs resto | Solo invoices usa `INVOICE_STATUS_LABELS`; resto inline | Inconsistencia, no i18n | `ui/src/modules/billing/<dom>StatusLabels.ts` | **Estandarizar constantes** (PR auxiliar, opcional) |
...
- **Coordinación**: requiere PR upstream al monorepo `core`. Si esto es out-of-scope, **mitigar localmente** con un helper en pymes:
  ```go
  // core/backend/internal/shared/fsmext/is_terminal.go
...
- **Path**: `core/backend/internal/shared/status/fsm_errors.go`
...
- **Path**: `core/backend/internal/shared/handlers/status_endpoint.go`
- **Justificación de ubicación**: depende de `RBACMiddleware`, `GetAuthContext`, `WriteValidation` que viven en el mismo paquete. Mover a `core/shared/backend/` introduciría back-dep.
...
- `core/backend/internal/sales/fsm.go`
- `core/backend/internal/quotes/fsm.go`
- `core/backend/internal/purchases/fsm.go`
- `core/backend/internal/invoices/fsm.go`
...
- **Archivo afectado**: `ui/src/modules/billing/billingHelpers.ts` (función actual `buildFullyConnectedStatusStateMachine`).
...
- **Nueva fuente de verdad** (por dominio): `ui/src/modules/billing/<dom>StateMachine.ts`:
  ```ts
...
- **Archivo nuevo**: `ui/src/lib/endpoints.ts`:
  ```ts
...
- **Archivo**: `ui/src/lib/invoicesApi.ts:190-192`
  ```ts
  // Antes:
  ...
  ```
- También en `ui/src/modules/billing/billingInvoicesConfig.ts`: quitar el `if (values.status !== undefined) body.status = ...` del builder de `Update` genérico. Status ya no entra por ahí.
...
| **C — Codegen desde FSM Go** | Script `tools/fsm-export/main.go` que importa los FSM de cada dominio y emite `ui/src/modules/billing/<dom>StateMachine.gen.ts`. CI corre el script y falla si el archivo está stale. | SoT en backend; FE consume tipos+constantes; sin endpoint runtime; requiere script + CI step. |
...
- **Archivos a crear**:
  - `core/backend/internal/shared/status/fsm_errors.go` (`MapFSMError`)
  - `core/backend/internal/shared/status/fsm_errors_test.go`
  - `core/backend/internal/shared/handlers/status_endpoint.go` (`RegisterStatusEndpoint[T]`)
  - `core/backend/internal/shared/handlers/status_endpoint_test.go`
...
- **Verificación**:
  ```bash
  cd /home/pablocristo/Proyectos/pablo/pymes
  go test ./core/backend/internal/shared/status/... -race -count=1
  go test ./core/backend/internal/shared/handlers/... -race -count=1
  go vet ./core/backend/internal/shared/...
  ```
- **Verificación de no back-dep**:
  ```bash
  rg "internal/shared/handlers" /home/pablocristo/Proyectos/pablo/pymes/core/backend/internal/shared/status/
  # debe estar VACÍO
  ```
...
- **Archivos a crear**: `core/backend/internal/sales/fsm.go`.
- **Archivos a modificar**:
  - `core/backend/internal/sales/usecases.go` (líneas 363-391, 446-453): refactor `UpdateStatus`. Eliminar `isValidSaleStatus`. Agregar `repo.GetByID`. Llamar `saleStateMachine.Validate`. Wrap con `handlers.MapFSMError`.
  - `core/backend/internal/sales/handler.go` (líneas 39, 264-291): eliminar método handler `UpdateStatus`. Reemplazar con `handlers.RegisterStatusEndpoint(...)`.
  - `core/backend/internal/sales/usecases_test.go`: agregar `TestSale_UpdateStatus_FSM` table-driven.
...
- **Verificación**:
  ```bash
  cd /home/pablocristo/Proyectos/pablo/pymes
  go test ./core/backend/internal/sales/... -race -count=1
  ```
...
- **Archivos a crear**: `core/backend/internal/quotes/fsm.go`.
- **Archivos a modificar**:
...
- **Verificación**:
  ```bash
  go test ./core/backend/internal/quotes/... -race -count=1
  ```
...
- **Archivos a crear**: `core/backend/internal/purchases/fsm.go`.
- **Archivos a modificar**:
...
- **Verificación**:
  ```bash
  cd /home/pablocristo/Proyectos/pablo/pymes
  go test ./core/backend/internal/purchases/... -race -count=1
  ```
...
- **Archivos a crear**:
  - `core/backend/internal/invoices/fsm.go`
  - `core/backend/internal/invoices/usecases_test.go` (no existe hoy)
- **Archivos a modificar**:
  ...
  - **Frontend**:
    - `ui/src/lib/invoicesApi.ts:190-192`: `updateInvoiceStatus` apunta a `/v1/invoices/${id}/status` (PATCH).
    - `ui/src/modules/billing/billingInvoicesConfig.ts` (~165): quitar el bloque que mete `status` en el body de Update genérico.
...
- **Verificación**:
  ```bash
  cd /home/pablocristo/Proyectos/pablo/pymes
  go test ./core/backend/internal/invoices/... -race -count=1
...
- **Archivos a crear**:
  - `ui/src/modules/billing/salesStateMachine.ts`
  - `ui/src/modules/billing/quotesStateMachine.ts`
  - `ui/src/modules/billing/purchasesStateMachine.ts`
  - `ui/src/modules/billing/invoicesStateMachine.ts`
- **Archivos a modificar**:
  - `ui/src/modules/billing/billingHelpers.ts`: deprecar `buildFullyConnectedStatusStateMachine` (mantener exportada si tests fallan; eliminar en cleanup).
  - `ui/src/modules/billing/billing<Sales|Quotes|Purchases|Invoices>Config.ts`: reemplazar el llamado a `buildFullyConnectedStatusStateMachine` con import del Builder espejo.
...
- **Archivos a crear**: `ui/src/lib/endpoints.ts`.
...
- **Archivos a crear**:
  - `core/backend/internal/e2e/status_workflows_test.go` — usa `httptest` o cliente HTTP real contra cp-backend con build tag `e2e`.
  - `core/backend/internal/e2e/helpers/db.go` — seed mínimo (org, user, party, sale/quote/purchase/invoice).
  - `core/backend/internal/e2e/helpers/auth.go` — token de prueba.
...
- **Verificación**:
  ...
  go test -tags e2e ./core/backend/internal/e2e/... -count=1 -v
  ```

#### 9.2 Playwright E2E (frontend)

- **Archivos a crear**:
  - `ui/e2e/billing-status-workflows.spec.ts`
  - `ui/e2e/helpers/login.ts` (reusa el login existente para `medlablocal`)
  - `ui/e2e/helpers/seedBillingData.ts` (vía API, crea sale/quote/purchase/invoice en estados conocidos)
- **Specs cubiertos**:
  ```ts
...
- **Verificación**:
  ...
  cd /home/pablocristo/Proyectos/pablo/pymes
  make up
  cd ui && npx playwright test e2e/billing-status-workflows.spec.ts
  ```
...
- **Archivos a crear**:
  - `core/backend/internal/sales/fsm_match_test.go` (tabla canónica de transiciones permitidas)
  - `core/backend/internal/quotes/fsm_match_test.go`
  - `core/backend/internal/purchases/fsm_match_test.go`
  - `core/backend/internal/invoices/fsm_match_test.go`
  - `ui/src/modules/billing/__tests__/<dom>StateMachine.shape.test.ts` para los 4
- Cada par (Go/TS) declara la MISMA tabla constante. Si el grafo cambia, ambos lados deben actualizar; el comentario `// MUST MATCH ui/src/modules/billing/<dom>StateMachine.ts` (y viceversa) facilita el code review.
...
- **Verificación**:
  ```bash
  cd /home/pablocristo/Proyectos/pablo/pymes
  rg "buildFullyConnectedStatusStateMachine" ui/src/  # debe ser 0
  rg "isValid.*Status|valid.*Statuses" core/backend/internal/{sales,quotes,purchases,invoices}/  # debe ser 0
...
### 9.2 Tests `core` por dominio (PR 3-6)
...
`core/backend/internal/shared/handlers/status_endpoint_test.go` con `httptest`:
...
`core/backend/internal/sales/fsm_match_test.go`:

```go
// Tabla canónica de pares válidos. Sincronizar con ui/src/modules/billing/salesStateMachine.test.ts
...
# Frontend
cd /home/pablocristo/Proyectos/pablo/pymes/ui
npx tsc --noEmit
npx vitest run
...
rg "isValid.*Status|valid.*Statuses" /home/pablocristo/Proyectos/pablo/pymes/core/backend/internal/{sales,quotes,purchases,invoices}/
...
rg "canTransition.*Status" /home/pablocristo/Proyectos/pablo/pymes/core/backend/internal/
# Después del refactor: 0 matches (purchases lo elimina)

# Endpoints /status que no usen el helper
rg "PATCH.*\".*status\"|/status\"" /home/pablocristo/Proyectos/pablo/pymes/core/backend/internal/{sales,quotes,purchases,invoices}/handler.go
...
rg "buildFullyConnectedStatusStateMachine" /home/pablocristo/Proyectos/pablo/pymes/ui/src/
...
# core/shared no debe importar core/internal (back-dep)
rg "core/backend/internal" /home/pablocristo/Proyectos/pablo/pymes/core/shared/
# debe estar vacío

# Usecases NO importan internal/shared/handlers (Sección 5.3.1 — separación capas)
rg "internal/shared/handlers" /home/pablocristo/Proyectos/pablo/pymes/core/backend/internal/{sales,quotes,purchases,invoices}/usecases.go
# debe estar vacío

# MapFSMError vive en status, NO en handlers
rg "func MapFSMError" /home/pablocristo/Proyectos/pablo/pymes/core/backend/internal/shared/handlers/
# debe estar vacío
rg "func MapFSMError" /home/pablocristo/Proyectos/pablo/pymes/core/backend/internal/shared/status/
# debe estar (1 match)
```
...
grep -E "\"@devpablocristo/" /home/pablocristo/Proyectos/pablo/pymes/ui/package.json
...
### Backend
- [ ] `rg "isValid.*Status|valid.*Statuses" core/backend/internal/{sales,quotes,purchases,invoices}/` devuelve 0 (excepto el normalizador de defaults en purchases.prepareCreate).
- [ ] `rg "canTransitionPurchaseStatus|canTransition.*Status" core/backend/internal/` devuelve 0.
...
- [ ] `MapFSMError` vive en `internal/shared/status/`, NO en `handlers/`. Verificar: `rg "MapFSMError" core/backend/internal/shared/handlers/` debe devolver 0.
- [ ] Usecases NO importan `internal/shared/handlers`: `rg "shared/handlers" core/backend/internal/{sales,quotes,purchases,invoices}/usecases.go` debe devolver 0.
...
- [ ] `go test ./core/backend/internal/{sales,quotes,purchases,invoices}/... -race -count=1` pasa.
- [ ] `go test ./core/backend/internal/shared/{status,handlers}/... -race -count=1` pasa.
...
### Frontend
- [ ] `rg "buildFullyConnectedStatusStateMachine" ui/src/` devuelve 0.
...
- [ ] `ui/src/lib/invoicesApi.ts:updateInvoiceStatus` apunta a `/v1/invoices/${id}/status`.
- [ ] `ui/src/modules/billing/billingInvoicesConfig.ts` ya no mete `status` en body de Update genérico.
...
- [ ] Cada FSM tiene un comentario `// MUST MATCH ui/src/modules/billing/<dom>StateMachine.ts` (y viceversa).
- [ ] Issue creado para fase futura de codegen / spec compartida (ver Sección 6.4).

### E2E (PR 9, OBLIGATORIO antes de cleanup)
- [ ] `go test -tags e2e ./core/backend/internal/e2e/... -count=1` pasa con DB efímera.
...
- [ ] `grep -E "\"@devpablocristo/[^\"]+\":\s*\"(file:|link:|\\.\\./)" ui/package.json` devuelve 0.
...
### Documentación
- [ ] `pymes/ARCHITECTURE.md` existe y describe core/modules/core/ui.
...
| `core/backend/internal/*` | Dominio del producto. Hexagonal: usecases + handler + repository | Importa core, modules; NO importa internal de otro vertical |
| `core/shared/backend/*` | Helpers transversales del producto (httperrors, ports, etc.) | NO depende de internal/* (back-dep prohibido) |
| `verticales/*` | Workshops, professionals, beauty, restaurants | Importa core, modules, HTTP API de core; NO domain interno de otro vertical |
| `ui/*` | Consola React | Importa @devpablocristo/{core,modules}-* desde npm |
...
1. Crear `core/backend/internal/<dom>/{usecases,handler,repository}.go`.
...
- **Status update**: `<dom>StateMachine` + `RegisterStatusEndpoint` + `MapFSMError`. Ver `core/backend/internal/sales/fsm.go` como referencia.
...
|---|---|---|
| Convivencia `archived_at` y `deleted_at` | Confirmada | `rg "archived_at\|deleted_at" core/backend/migrations/*.sql` |
...
| `core/http/go/pagination` ya extraído | Confirmado | `core/backend/internal/shared/handlers/pagination.go:7` |
| Pyme no importa pymes desde core ni modules | Confirmado | `rg "github.com/devpablocristo/pymes" core/ modules/` → 0 |
| 36 dominios CRUD identificados | Confirmado | `ls core/backend/internal` |
| `buildFullyConnectedStatusStateMachine` usado en 4 configs | Confirmado | `rg "buildFullyConnectedStatusStateMachine" ui/src/` |
| `core-fsm` solo usado en 1 archivo en frontend | Confirmado | `rg "@devpablocristo/core-fsm" ui/src/` |
| 24 dominios usan AuditPort identical | Confirmado | grep en `core/backend/internal/*/usecases.go` |
