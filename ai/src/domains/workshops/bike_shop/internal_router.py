...
async def chat_bike_shop(
    ...
    logger.info("bike_shop_chat_started", tenant_id=auth.tenant_id, user_id=auth.actor)

    declarations, handlers = build_internal_tools(backend_client, auth)
    llm_messages: list[Message] = [
        Message(role="system", content=build_system_prompt("internal", {"actor": auth.actor, "role": auth.role, "tenant_name": "la bicicleteria"})),
