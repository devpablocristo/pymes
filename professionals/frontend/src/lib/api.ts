...
import type { ProfessionalProfile, Specialty, ServiceLink, Intake, Session, SessionNote } from './types';
...
export async function getIntakes(): Promise<{ items: Intake[] }> {
  return professionalRequest('/v1/intakes');
}

export async function getIntake(id: string): Promise<Intake> {
  return professionalRequest(`/v1/intakes/${id}`);
}
...
export async function createIntake(data: {
  ...
  return professionalRequest('/v1/intakes', { method: 'POST', body: data });
}
...
export async function updateIntake(
  ...
  return professionalRequest(`/v1/intakes/${id}`, { method: 'PUT', body: data });
}

export async function submitIntake(id: string): Promise<Intake> {
  return professionalRequest(`/v1/intakes/${id}/submit`, { method: 'POST', body: {} });
}
