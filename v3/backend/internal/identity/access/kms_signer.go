...
func (s *kmsEd25519Signer) Sign(ctx context.Context, data []byte) ([]byte, error) {
	...
	if !validCRC32C(response.GetSignatureCrc32C(), response.GetSignature()) {
...
func loadKMSVerificationKey(ctx context.Context, client KMSClient, keyVersion string) (VerificationKey, error) {
	...
	if !validCRC32C(response.GetPemCrc32C(), []byte(response.GetPem())) {
	...
	return VerificationKey{KeyID: stableKMSKeyID(public), PublicKey: public}, nil
}
