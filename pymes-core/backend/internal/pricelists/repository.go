...
func (r *Repository) List(ctx context.Context, tenantID uuid.UUID, activeOnly bool, limit int) ([]pricelistdomain.PriceList, error) {
	limit = pagination.NormalizeLimit(limit, pagination.Config{DefaultLimit: 20, MaxLimit: 100})
	q := r.db.WithContext(ctx).Model(&models.PriceListModel{}).Where("tenant_id = ? AND deleted_at IS NULL", tenantID)
	if activeOnly {
...
func (r *Repository) ListArchived(ctx context.Context, tenantID uuid.UUID, limit int) ([]pricelistdomain.PriceList, error) {
	limit = pagination.NormalizeLimit(limit, pagination.Config{DefaultLimit: 20, MaxLimit: 100})
	var rows []models.PriceListModel
...
func (r *Repository) Create(ctx context.Context, in pricelistdomain.PriceList) (pricelistdomain.PriceList, error) {
	...
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		if in.IsDefault {
			if err := tx.Model(&models.PriceListModel{}).Where("tenant_id = ?", in.TenantID).Update("is_default", false).Error; err != nil {
				return err
			}
		}
		row := models.PriceListModel{ID: in.ID, TenantID: in.TenantID, Name: in.Name, Description: in.Description, IsDefault: in.IsDefault, Markup: in.Markup, IsActive: in.IsActive, IsFavorite: in.IsFavorite, Tags: pq.StringArray(utils.NormalizeTags(in.Tags)), CreatedAt: time.Now().UTC(), UpdatedAt: time.Now().UTC()}
...
func (r *Repository) GetByID(ctx context.Context, tenantID, id uuid.UUID) (pricelistdomain.PriceList, error) {
...
func (r *Repository) Update(ctx context.Context, in pricelistdomain.PriceList) (pricelistdomain.PriceList, error) {
	...
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		if in.IsDefault {
			if err := tx.Model(&models.PriceListModel{}).Where("tenant_id = ? AND id <> ?", in.TenantID, in.ID).Update("is_default", false).Error; err != nil {
				return err
			}
		}
		if err := tx.Model(&models.PriceListModel{}).Where("tenant_id = ? AND id = ? AND deleted_at IS NULL", in.TenantID, in.ID).Updates(map[string]any{"name": in.Name, "description": in.Description, "is_default": in.IsDefault, "markup": in.Markup, "is_active": in.IsActive, "is_favorite": in.IsFavorite, "tags": pq.StringArray(utils.NormalizeTags(in.Tags)), "updated_at": time.Now().UTC()}).Error; err != nil {
			return err
		}
		...
		updated, err := r.GetByID(ctx, in.TenantID, in.ID)
		if err != nil {
			return err
		}
		out = updated
...
func (r *Repository) HardDelete(ctx context.Context, tenantID, id uuid.UUID) error {
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		if err := tx.Where("price_list_id = ?", id).Delete(&models.PriceListItemModel{}).Error; err != nil {
			return err
		}
		if err := tx.Where("price_list_id = ?", id).Delete(&models.ServicePriceListItemModel{}).Error; err != nil {
			return err
		}
		res := tx.Where("tenant_id = ? AND id = ?", tenantID, id).Delete(&models.PriceListModel{})
		if res.Error != nil {
			return res.Error
		}
		if res.RowsAffected == 0 {
...
func modelToDomain(row models.PriceListModel, items []models.PriceListItemModel, serviceItems []models.ServicePriceListItemModel) pricelistdomain.PriceList {
	out := pricelistdomain.PriceList{ID: row.ID, TenantID: row.TenantID, Name: row.Name, Description: row.Description, IsDefault: row.IsDefault, Markup: row.Markup, IsActive: row.IsActive, IsFavorite: row.IsFavorite, Tags: append([]string(nil), row.Tags...), ArchivedAt: row.DeletedAt, CreatedAt: row.CreatedAt, UpdatedAt: row.UpdatedAt}
	for _, item := range items {
