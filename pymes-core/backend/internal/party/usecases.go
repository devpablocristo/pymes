...
type RepositoryPort interface {
	...
	AddRole(ctx context.Context, tenantID, partyID uuid.UUID, in partydomain.PartyRole) (partydomain.PartyRole, error)
	RemoveRole(ctx context.Context, tenantID, partyID uuid.UUID, role string) error
...
func (u *Usecases) Create(ctx context.Context, in partydomain.Party, actor string) (partydomain.Party, error) {
	...
	if u.audit != nil {
		u.audit.Log(ctx, out.TenantID.String(), actor, "party.created", "party", out.ID.String(), map[string]any{"party_type": out.PartyType, "display_name": out.DisplayName})
	}
	if u.timeline != nil {
		_ = u.timeline.RecordEvent(ctx, out.TenantID, "parties", out.ID, "party.created", "Party creada", out.DisplayName, actor, map[string]any{"party_type": out.PartyType})
	}
	if u.webhooks != nil {
		_ = u.webhooks.Enqueue(ctx, out.TenantID, "party.created", map[string]any{"party_id": out.ID.String(), "party_type": out.PartyType, "display_name": out.DisplayName})
	}
	return out, nil
}

func (u *Usecases) GetByID(ctx context.Context, tenantID, id uuid.UUID) (partydomain.Party, error) {
...
func (u *Usecases) Update(ctx context.Context, tenantID, id uuid.UUID, in partydomain.Party, actor string) (partydomain.Party, error) {
	in.ID = id
	...
	if u.audit != nil {
		u.audit.Log(ctx, tenantID.String(), actor, "party.updated", "party", id.String(), map[string]any{"party_type": out.PartyType, "display_name": out.DisplayName})
	}
	if u.timeline != nil {
		_ = u.timeline.RecordEvent(ctx, tenantID, "parties", id, "party.updated", "Party actualizada", out.DisplayName, actor, map[string]any{"party_type": out.PartyType})
	}
	if u.webhooks != nil {
		_ = u.webhooks.Enqueue(ctx, tenantID, "party.updated", map[string]any{"party_id": id.String(), "party_type": out.PartyType, "display_name": out.DisplayName})
	}
	return out, nil
}
...
func (u *Usecases) Delete(ctx context.Context, tenantID, id uuid.UUID, actor string) error {
	...
	if u.audit != nil {
		u.audit.Log(ctx, tenantID.String(), actor, "party.deleted", "party", id.String(), map[string]any{})
	}
	if u.timeline != nil {
		_ = u.timeline.RecordEvent(ctx, tenantID, "parties", id, "party.deleted", "Party eliminada", id.String(), actor, map[string]any{})
	}
	if u.webhooks != nil {
		_ = u.webhooks.Enqueue(ctx, tenantID, "party.deleted", map[string]any{"party_id": id.String()})
	}
	return nil
}

func (u *Usecases) AddRole(ctx context.Context, tenantID, partyID uuid.UUID, role string, priceListID *uuid.UUID, metadata map[string]any, actor string) (partydomain.PartyRole, error) {
	role = strings.TrimSpace(role)
	if role == "" {
		return partydomain.PartyRole{}, domainerr.Validation("role is required")
	}
	out, err := u.repo.AddRole(ctx, tenantID, partyID, partydomain.PartyRole{Role: role, IsActive: true, PriceListID: priceListID, Metadata: metadata})
	if err != nil {
		return partydomain.PartyRole{}, translateRepoErr(err)
	}
	if u.audit != nil {
		u.audit.Log(ctx, tenantID.String(), actor, "party.role_added", "party", partyID.String(), map[string]any{"role": role})
	}
	if u.timeline != nil {
		_ = u.timeline.RecordEvent(ctx, tenantID, "parties", partyID, "party.role_added", "Rol agregado", role, actor, map[string]any{"role": role})
	}
	return out, nil
}

func (u *Usecases) RemoveRole(ctx context.Context, tenantID, partyID uuid.UUID, role, actor string) error {
	if err := u.repo.RemoveRole(ctx, tenantID, partyID, role); err != nil {
		return translateRepoErr(err)
	}
	if u.audit != nil {
		u.audit.Log(ctx, tenantID.String(), actor, "party.role_removed", "party", partyID.String(), map[string]any{"role": role})
	}
	if u.timeline != nil {
		_ = u.timeline.RecordEvent(ctx, tenantID, "parties", partyID, "party.role_removed", "Rol removido", role, actor, map[string]any{"role": role})
	}
	return nil
}

func (u *Usecases) ListRelationships(ctx context.Context, tenantID, partyID uuid.UUID) ([]partydomain.PartyRelationship, error) {
	out, err := u.repo.ListRelationships(ctx, tenantID, partyID)
	if err != nil {
...
func (u *Usecases) CreateRelationship(ctx context.Context, in partydomain.PartyRelationship, actor string) (partydomain.PartyRelationship, error) {
	...
	if u.audit != nil {
		u.audit.Log(ctx, in.TenantID.String(), actor, "party.relationship_created", "party_relationship", out.ID.String(), map[string]any{"relationship_type": out.RelationshipType})
	}
	if u.webhooks != nil {
		_ = u.webhooks.Enqueue(ctx, in.TenantID, "party.relationship_created", map[string]any{"relationship_id": out.ID.String(), "from_party_id": out.FromPartyID.String(), "to_party_id": out.ToPartyID.String(), "relationship_type": out.RelationshipType})
	}
	return out, nil
}
