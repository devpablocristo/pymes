...
func TestPostgresProjectsClerkOrganizationAndMembershipIdempotently(t *testing.T) {
	...
	if duplicate, err := repo.Receive(context.Background(), usecases.Event{ID: "evt-org", Type: "organization.created", OccurredAt: now, Payload: organization}); err != nil || duplicate {
	...
	if duplicate, err := repo.Receive(context.Background(), usecases.Event{ID: "evt-member", Type: "organizationMembership.created", OccurredAt: now, Payload: membership}); err != nil || duplicate {
	...
	if duplicate, err := repo.Receive(context.Background(), usecases.Event{ID: "evt-org-updated", Type: "organization.updated", OccurredAt: now, Payload: organizationUpdated}); err != nil || duplicate {
	...
	if duplicate, err := repo.Receive(context.Background(), usecases.Event{ID: "evt-member-deleted", Type: "organizationMembership.deleted", OccurredAt: now, Payload: membershipDeleted}); err != nil || duplicate {
