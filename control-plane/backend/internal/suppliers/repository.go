...
import (
	...
	"github.com/devpablocristo/pymes/control-plane/backend/internal/suppliers/repository/models"
...
func (r *Repository) List(ctx context.Context, p ListParams) ([]supplierdomain.Supplier, int64, bool, *uuid.UUID, error) {
	...
	q := r.db.WithContext(ctx).Model(&models.SupplierModel{}).Where("org_id = ? AND deleted_at IS NULL", p.OrgID)
	if tag := strings.TrimSpace(p.Tag); tag != "" {
		q = q.Where("? = ANY(tags)", tag)
	}
	if s := strings.TrimSpace(p.Search); s != "" {
		like := "%" + s + "%"
		q = q.Where("(name ILIKE ? OR email ILIKE ? OR phone ILIKE ? OR tax_id ILIKE ? OR contact_name ILIKE ?)", like, like, like, like, like)
	}
	var total int64
...
func (r *Repository) Update(ctx context.Context, in supplierdomain.Supplier) (supplierdomain.Supplier, error) {
	...
	res := r.db.WithContext(ctx).Model(&models.SupplierModel{}).
		Where("org_id = ? AND id = ? AND deleted_at IS NULL", in.OrgID, in.ID).
		Updates(updates)
	if res.Error != nil {
		return supplierdomain.Supplier{}, res.Error
	}
...
func toDomain(row models.SupplierModel) supplierdomain.Supplier {
