...
func (c *Client) Get(ctx context.Context, path, tenantID string) (map[string]any, error) {
	opts := c.headerOpts(tenantID)
...
func (c *Client) Post(ctx context.Context, path, tenantID string, payload any) (map[string]any, error) {
	opts := c.headerOpts(tenantID)
...
func (c *Client) headerOpts(tenantID string) []httpclient.RequestOption {
