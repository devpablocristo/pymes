...
export function NotificationsCenterPage({ embedded = false }: NotificationsCenterPageProps) {
  ...
  const summaryBadge = useMemo(() => {
    ...
    if (unreadCount > 0) {
      parts.push(
        unreadCount === 1
          ? `1 ${t('ai.notifications.summary.notification')} ${t('ai.notifications.summary.unreadSuffix')}`
          : `${unreadCount} ${t('ai.notifications.summary.notifications')} ${t('ai.notifications.summary.unreadSuffix')}`,
      );
    }
  ...
  return (
    <PageLayout
      title={t('ai.notifications.pageTitle')}
      lead={
        <>
          <div>{t('ai.notifications.pageLead')}</div>
          <div className="text-secondary">{summaryBadge}</div>
        </>
      }
    >
      {feed}
