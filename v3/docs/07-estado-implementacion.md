...
| Rollout tenant | Configuración central con cuatro flags seguros por default, RLS, versión optimista y auditoría inmutable; enforcement en API pública/administrativa y workers de Agenda, PerGo, Google y Fiscal real. | Tests unitarios y PostgreSQL; contrato OpenAPI y [runbook](12-feature-flags.md). |
...
| Notifications/PerGo | Proyección consumer-owned de eventos Scheduling, intenciones tenant, ruta no secreta congelada por organización, feature flag, outbox, adapter PerGo, idempotencia durable en PerGo, trace ID tenant-aware, estados monotónicos, webhook HMAC con rotación e inbox inmutable; teléfono, cuerpo y variables redactados. | Tests Go/PostgreSQL y `make notifications-e2e`; [diseño y runbook específico](11-notifications-pergo.md). |
