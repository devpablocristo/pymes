...
type RepositoryPort interface {
	...
	Create(ctx context.Context, in Vehicle) (Vehicle, error)
	GetByID(ctx context.Context, orgID, id uuid.UUID) (Vehicle, error)
	Update(ctx context.Context, in Vehicle) (Vehicle, error)
}
...
func (u *Usecases) List(ctx context.Context, p ListParams) ([]Vehicle, int64, bool, *uuid.UUID, error) {
	return u.repo.List(ctx, p)
}

func (u *Usecases) Create(ctx context.Context, in Vehicle, actor string) (Vehicle, error) {
...
func (u *Usecases) GetByID(ctx context.Context, orgID, id uuid.UUID) (Vehicle, error) {
	out, err := u.repo.GetByID(ctx, orgID, id)
	if err != nil {
		if err == gorm.ErrRecordNotFound {
			return Vehicle{}, fmt.Errorf("vehicle not found: %w", httperrors.ErrNotFound)
		}
		return Vehicle{}, err
	}
	return out, nil
...
func (u *Usecases) Update(ctx context.Context, orgID, id uuid.UUID, in UpdateInput, actor string) (Vehicle, error) {
...
func (u *Usecases) enrichCustomer(ctx context.Context, in *Vehicle) error {
	if in.CustomerID == nil {
		return nil
	}
	if u.cp == nil {
...
func validateVehicle(in *Vehicle) error {
