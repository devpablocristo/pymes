...
import (
	...
	"github.com/devpablocristo/pymes/workshops/backend/internal/bike_shop/workshopservices/repository/models"
...
func (r *Repository) Create(ctx context.Context, in domain.Service) (domain.Service, error) {
	row := models.ServiceModel{
		ID:              uuid.New(),
		OrgID:           in.OrgID,
		Segment:         workshopshared.SegmentBikeShop,
		Code:            in.Code,
		Name:            in.Name,
...
func (r *Repository) GetByID(ctx context.Context, orgID, id uuid.UUID) (domain.Service, error) {
	...
	if err := r.db.WithContext(ctx).Where("org_id = ? AND id = ? AND segment = ?", orgID, id, workshopshared.SegmentBikeShop).Take(&row).Error; err != nil {
