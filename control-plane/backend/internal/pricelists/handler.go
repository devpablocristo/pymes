...
func (h *Handler) Delete(c *gin.Context) {
	orgID, id, ok := parseOrgID(c)
	if !ok { return }
	if err := h.uc.Delete(c.Request.Context(), orgID, id); err != nil { httperrors.Respond(c, err); return }
...
func requestToDomain(orgID uuid.UUID, req dto.CreatePriceListRequest) pricelistdomain.PriceList {
	...
	for _, item := range req.Items {
		if productID, err := uuid.Parse(strings.TrimSpace(item.ProductID)); err == nil { items = append(items, pricelistdomain.PriceListItem{ProductID: productID, Price: item.Price}) }
	}
