...
func (r *internalAPIKeyResolver) ResolveAPIKey(raw string) (users.ResolvedAPIKey, bool) {
	...
	var kr row
	if err := r.db.Table("tenant_api_keys").Select("id", "tenant_id").Where("api_key_hash = ?", hash).Take(&kr).Error; err != nil {
		return users.ResolvedAPIKey{}, false
	}
	var scopes []string
	_ = r.db.Table("tenant_api_key_scopes").Where("api_key_id = ?", kr.ID).Order("scope").Pluck("scope", &scopes)
	sort.Strings(scopes)
	return users.ResolvedAPIKey{ID: kr.ID, TenantID: kr.TenantID, Scopes: scopes}, true
}
