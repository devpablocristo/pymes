...
func (r *Repository) List(ctx context.Context, p ListParams) ([]Service, int64, bool, *uuid.UUID, error) {
	limit := pagination.NormalizeLimit(p.Limit, 20, 100)
	q := r.db.WithContext(ctx).Model(&ServiceModel{}).Where("org_id = ?", p.OrgID)
...
func (r *Repository) Create(ctx context.Context, in Service) (Service, error) {
	row := ServiceModel{
		ID:              uuid.New(),
		OrgID:           in.OrgID,
		Code:            in.Code,
...
func (r *Repository) GetByID(ctx context.Context, orgID, id uuid.UUID) (Service, error) {
	var row ServiceModel
...
func (r *Repository) Update(ctx context.Context, in Service) (Service, error) {
	...
	res := r.db.WithContext(ctx).Model(&ServiceModel{}).
...
func toDomain(row ServiceModel) Service {
	return Service{
		ID:              row.ID,
		OrgID:           row.OrgID,
		Code:            row.Code,
