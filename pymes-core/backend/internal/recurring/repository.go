...
func (r *Repository) List(ctx context.Context, tenantID uuid.UUID, activeOnly bool, limit int) ([]recurringdomain.RecurringExpense, error) {
	limit = pagination.NormalizeLimit(limit, pagination.Config{DefaultLimit: 20, MaxLimit: 100})
	q := r.db.WithContext(ctx).Model(&models.RecurringExpenseModel{}).Where("tenant_id = ? AND deleted_at IS NULL", tenantID)
	if activeOnly {
...
func (r *Repository) ListArchived(ctx context.Context, tenantID uuid.UUID, limit int) ([]recurringdomain.RecurringExpense, error) {
	limit = pagination.NormalizeLimit(limit, pagination.Config{DefaultLimit: 20, MaxLimit: 100})
	var rows []models.RecurringExpenseModel
...
func (r *Repository) GetByID(ctx context.Context, tenantID, id uuid.UUID) (recurringdomain.RecurringExpense, error) {
...
func (r *Repository) Deactivate(ctx context.Context, tenantID, id uuid.UUID) error {
	res := r.db.WithContext(ctx).Model(&models.RecurringExpenseModel{}).Where("tenant_id = ? AND id = ? AND deleted_at IS NULL", tenantID, id).Updates(map[string]any{"is_active": false, "updated_at": time.Now().UTC()})
	if res.Error != nil {
		return res.Error
	}
...
func (r *Repository) HardDelete(ctx context.Context, tenantID, id uuid.UUID) error {
	res := r.db.WithContext(ctx).Where("tenant_id = ? AND id = ?", tenantID, id).Delete(&models.RecurringExpenseModel{})
	if res.Error != nil {
		return res.Error
	}
	if res.RowsAffected == 0 {
...
func (r *Repository) GetCurrency(ctx context.Context, tenantID uuid.UUID) string {
	var cur string
	if err := r.db.WithContext(ctx).Table("tenant_settings").Select("currency").Where("tenant_id = ?", tenantID).Take(&cur).Error; err != nil || cur == "" {
		return "ARS"
	}
	return cur
}
