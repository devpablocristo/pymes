...
func (s *IssuerTokenSource) Token(ctx context.Context, audience, organizationID string) (string, error) {
	...
	if principal, ok := identityusecases.PrincipalFromContext(ctx); ok && principal.CanRead(organizationID) {
		actorID = principal.ActorID
		if delegated, exists := identityusecases.DelegatedActorFromContext(ctx); exists {
...
func tokenSourceFromRuntime(
	...
	case "production":
		...
		serviceIssuer, closer, err := newCloudKMSIssuer(
			...
			issuer,
			keyVersion,
			csvValues(getenv("PYMES_INTERNAL_KMS_OVERLAP_KEY_VERSIONS")),
		)
		if err != nil {
			return nil, err
		}
