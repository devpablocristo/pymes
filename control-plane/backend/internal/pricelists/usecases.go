...
func (u *Usecases) List(ctx context.Context, orgID uuid.UUID, activeOnly bool, limit int) ([]pricelistdomain.PriceList, error) { return u.repo.List(ctx, orgID, activeOnly, limit) }

func (u *Usecases) Create(ctx context.Context, in pricelistdomain.PriceList) (pricelistdomain.PriceList, error) {
	if strings.TrimSpace(in.Name) == "" { return pricelistdomain.PriceList{}, apperror.NewBadInput("name is required") }
	if in.ID == uuid.Nil { in.ID = uuid.New() }
...
func (u *Usecases) GetByID(ctx context.Context, orgID, id uuid.UUID) (pricelistdomain.PriceList, error) {
	...
	if err != nil { if errors.Is(err, gorm.ErrRecordNotFound) { return pricelistdomain.PriceList{}, apperror.NewNotFound("price_list", id.String()) }; return pricelistdomain.PriceList{}, err }
	return out, nil
}
...
func (u *Usecases) Delete(ctx context.Context, orgID, id uuid.UUID) error { return u.repo.Delete(ctx, orgID, id) }
