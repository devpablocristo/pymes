...
DECLARE
    ...
    wo_bike uuid := uuid_generate_v5(v_org, 'pymes-seed/v1/workshop/wo/bike/1');
BEGIN
    DELETE FROM workshops.work_order_items WHERE work_order_id IN (wo1, wo2, wo_bike);
    DELETE FROM workshops.work_orders WHERE id IN (wo1, wo2, wo_bike);
