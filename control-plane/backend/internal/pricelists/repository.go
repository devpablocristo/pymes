...
import (
	...
	"github.com/devpablocristo/pymes/control-plane/backend/internal/pricelists/repository/models"
...
func (r *Repository) List(ctx context.Context, orgID uuid.UUID, activeOnly bool, limit int) ([]pricelistdomain.PriceList, error) {
	...
	if err := q.Order("is_default DESC").Order("name ASC").Limit(limit).Find(&rows).Error; err != nil { return nil, err }
	out := make([]pricelistdomain.PriceList, 0, len(rows))
	for _, row := range rows { out = append(out, modelToDomain(row, nil)) }
	return out, nil
}
...
func (r *Repository) Create(ctx context.Context, in pricelistdomain.PriceList) (pricelistdomain.PriceList, error) {
	...
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		if in.IsDefault { if err := tx.Model(&models.PriceListModel{}).Where("org_id = ?", in.OrgID).Update("is_default", false).Error; err != nil { return err } }
	...
	if err != nil { return pricelistdomain.PriceList{}, err }
	return out, nil
}
...
func (r *Repository) GetByID(ctx context.Context, orgID, id uuid.UUID) (pricelistdomain.PriceList, error) {
	...
	if err := r.db.WithContext(ctx).Where("org_id = ? AND id = ?", orgID, id).Take(&row).Error; err != nil { return pricelistdomain.PriceList{}, err }
	var items []models.PriceListItemModel
	if err := r.db.WithContext(ctx).Where("price_list_id = ?", id).Find(&items).Error; err != nil { return pricelistdomain.PriceList{}, err }
...
func (r *Repository) Update(ctx context.Context, in pricelistdomain.PriceList) (pricelistdomain.PriceList, error) {
	var out pricelistdomain.PriceList
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		if in.IsDefault { if err := tx.Model(&models.PriceListModel{}).Where("org_id = ? AND id <> ?", in.OrgID, in.ID).Update("is_default", false).Error; err != nil { return err } }
		if err := tx.Model(&models.PriceListModel{}).Where("org_id = ? AND id = ?", in.OrgID, in.ID).Updates(map[string]any{"name": in.Name, "description": in.Description, "is_default": in.IsDefault, "markup": in.Markup, "is_active": in.IsActive, "updated_at": time.Now().UTC()}).Error; err != nil { return err }
		if err := tx.Where("price_list_id = ?", in.ID).Delete(&models.PriceListItemModel{}).Error; err != nil { return err }
		items := toItemModels(in.ID, in.Items)
		if len(items) > 0 { if err := tx.Create(&items).Error; err != nil { return err } }
...
func toItemModels(priceListID uuid.UUID, items []pricelistdomain.PriceListItem) []models.PriceListItemModel {
	...
	for _, item := range items { rows = append(rows, models.PriceListItemModel{PriceListID: priceListID, ProductID: item.ProductID, Price: item.Price}) }
	return rows
}
...
func modelToDomain(row models.PriceListModel, items []models.PriceListItemModel) pricelistdomain.PriceList {
	...
	for _, item := range items { out.Items = append(out.Items, pricelistdomain.PriceListItem{ProductID: item.ProductID, Price: item.Price}) }
	return out
}
