...
func (r *Repository) List(ctx context.Context, p ListParams) ([]domain.WorkOrder, int64, bool, *uuid.UUID, error) {
	...
	q := r.db.WithContext(ctx).Model(&models.BikeWorkOrderModel{}).Where("org_id = ?", p.OrgID)
	if status := strings.TrimSpace(p.Status); status != "" {
		q = q.Where("status = ?", status)
	}
	if search := strings.TrimSpace(p.Search); search != "" {
		like := "%" + search + "%"
		q = q.Where("(number ILIKE ? OR bicycle_label ILIKE ? OR customer_name ILIKE ? OR requested_work ILIKE ?)", like, like, like, like)
	}
	var total int64
	...
	var rows []models.BikeWorkOrderModel
...
func (r *Repository) Create(ctx context.Context, in domain.WorkOrder) (domain.WorkOrder, error) {
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		row := models.BikeWorkOrderModel{
			ID:               uuid.New(),
			OrgID:            in.OrgID,
			Number:           in.Number,
			BicycleID:        in.BicycleID,
			BicycleLabel:     in.BicycleLabel,
...
func (r *Repository) GetByID(ctx context.Context, orgID, id uuid.UUID) (domain.WorkOrder, error) {
	var row models.BikeWorkOrderModel
...
func (r *Repository) Update(ctx context.Context, in domain.WorkOrder) (domain.WorkOrder, error) {
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		updates := map[string]any{
			"bicycle_id":        in.BicycleID,
			"bicycle_label":     in.BicycleLabel,
		...
		res := tx.Model(&models.BikeWorkOrderModel{}).Where("org_id = ? AND id = ?", in.OrgID, in.ID).Updates(updates)
		if res.Error != nil {
			return res.Error
		}
...
func (r *Repository) SaveIntegrations(ctx context.Context, orgID, id uuid.UUID, quoteID, saleID *uuid.UUID, status *string) (domain.WorkOrder, error) {
	...
	res := r.db.WithContext(ctx).Model(&models.BikeWorkOrderModel{}).Where("org_id = ? AND id = ?", orgID, id).Updates(updates)
	if res.Error != nil {
...
func (r *Repository) replaceItems(ctx context.Context, tx *gorm.DB, orgID, workOrderID uuid.UUID, items []domain.WorkOrderItem) error {
	if err := tx.WithContext(ctx).Where("org_id = ? AND work_order_id = ?", orgID, workOrderID).Delete(&models.BikeWorkOrderItemModel{}).Error; err != nil {
		return err
	}
	for index, item := range items {
		metadata, _ := json.Marshal(item.Metadata)
		row := models.BikeWorkOrderItemModel{
			ID:          uuid.New(),
			OrgID:       orgID,
...
func (r *Repository) loadItems(ctx context.Context, orgID, workOrderID uuid.UUID) ([]domain.WorkOrderItem, error) {
	var rows []models.BikeWorkOrderItemModel
...
func itemToDomain(row models.BikeWorkOrderItemModel) domain.WorkOrderItem {
...
func toDomain(row models.BikeWorkOrderModel, items []domain.WorkOrderItem) domain.WorkOrder {
	return domain.WorkOrder{
		ID:               row.ID,
		OrgID:            row.OrgID,
		Number:           row.Number,
		BicycleID:        row.BicycleID,
		BicycleLabel:     row.BicycleLabel,
