...
import (
	...
	"github.com/devpablocristo/pymes/v2/backend/internal/iamoutbox"
)

const serviceName = "pymes-v2-iam-worker"

func main() {
	logger := observability.NewJSONLogger(serviceName)
	if err := run(logger, os.Getenv); err != nil {
		logger.Error("IAM worker stopped", "event", "iam_worker_stopped", "error", err)
		os.Exit(1)
	}
}
...
type environment struct {
	...
	ClerkSecret  string
	ClerkBaseURL string
	Worker       iamoutbox.WorkerConfig
}

func run(logger *slog.Logger, getenv func(string) string) error {
	if logger == nil {
		return fmt.Errorf("IAM worker logger is required")
	}
	...
	worker, err := iamoutbox.NewPostgresWorker(database.Pool(), provider, cfg.Worker)
	if err != nil {
		return err
	}

	logger.Info("IAM worker started", "event", "iam_worker_started")
	return worker.Run(ctx, func(result platformoutbox.DispatchResult, dispatchErr error) {
		if dispatchErr != nil {
			logger.Error(
				"IAM outbox dispatch degraded",
				"event", "iam_outbox_dispatch_failed",
		...
		if result.Leased > 0 {
			logger.Info(
				"IAM outbox batch dispatched",
				"event", "iam_outbox_dispatched",
				"leased", result.Leased,
				"published", result.Published,
			)
		}
...
func loadEnvironment(getenv func(string) string) (environment, error) {
	if getenv == nil {
		return environment{}, fmt.Errorf("IAM worker environment reader is required")
	}
	cfg := environment{
		...
		Worker:       iamoutbox.DefaultWorkerConfig(),
	}
	if cfg.DatabaseURL == "" {
	...
	cfg.Worker.BatchSize, err = positiveInt(
		getenv("PYMES_IAM_WORKER_BATCH_SIZE"),
		cfg.Worker.BatchSize,
	)
	if err != nil {
		return environment{}, fmt.Errorf("PYMES_IAM_WORKER_BATCH_SIZE: %w", err)
	}
	cfg.Worker.PollInterval, err = positiveDuration(
		getenv("PYMES_IAM_WORKER_POLL_INTERVAL"),
		cfg.Worker.PollInterval,
	)
	if err != nil {
		return environment{}, fmt.Errorf("PYMES_IAM_WORKER_POLL_INTERVAL: %w", err)
	}
	cfg.Worker.LeaseDuration, err = positiveDuration(
		getenv("PYMES_IAM_WORKER_LEASE_DURATION"),
		cfg.Worker.LeaseDuration,
	)
	if err != nil {
		return environment{}, fmt.Errorf("PYMES_IAM_WORKER_LEASE_DURATION: %w", err)
	}
	cfg.Worker.PublishTimeout, err = positiveDuration(
		getenv("PYMES_IAM_WORKER_PUBLISH_TIMEOUT"),
		cfg.Worker.PublishTimeout,
	)
	if err != nil {
		return environment{}, fmt.Errorf("PYMES_IAM_WORKER_PUBLISH_TIMEOUT: %w", err)
	}
	return cfg, nil
}
