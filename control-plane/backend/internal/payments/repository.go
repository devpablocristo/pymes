...
func (r *Repository) CreateSalePayment(ctx context.Context, orgID, saleID uuid.UUID, in paymentsdomain.Payment) (paymentsdomain.Payment, error) {
	...
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Table("sales").Select("total, amount_paid, currency, payment_method").Where("org_id = ? AND id = ?", orgID, saleID).Take(&sale).Error; err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) { return apperror.NewNotFound("sale", saleID.String()) }
			return err
		}
		...
		if err := tx.Exec(`
			...
			VALUES (?, ?, 'sale', ?, ?, ?, ?, ?, ?, ?)
		`, paymentID, orgID, saleID, in.Method, in.Amount, in.Notes, in.ReceivedAt.UTC(), in.CreatedBy, createdAt).Error; err != nil { return err }
		newAmountPaid := sale.AmountPaid + in.Amount
		paymentStatus := "partial"
		if newAmountPaid >= sale.Total { paymentStatus = "paid" }
		paymentMethod := sale.PaymentMethod
		if paymentMethod == "" { paymentMethod = in.Method }
		if paymentMethod != in.Method { paymentMethod = "mixed" }
		if err := tx.Exec(`
			UPDATE sales SET amount_paid = ?, payment_status = ?, payment_method = ? WHERE org_id = ? AND id = ?
		`, newAmountPaid, paymentStatus, paymentMethod, orgID, saleID).Error; err != nil { return err }
		if err := tx.Exec(`
		...
		`, orgID, in.Amount, sale.Currency, defaultString(in.Notes, "sale payment"), in.Method, saleID, in.CreatedBy).Error; err != nil { return err }
	...
	if err != nil { return paymentsdomain.Payment{}, err }
	return out, nil
}
