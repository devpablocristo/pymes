...
CREATE INDEX IF NOT EXISTS idx_appointments_archived ON appointments (tenant_id, archived_at) WHERE archived_at IS NOT NULL;

ALTER TABLE quotes ADD COLUMN IF NOT EXISTS archived_at TIMESTAMPTZ;
CREATE INDEX IF NOT EXISTS idx_quotes_archived ON quotes (tenant_id, archived_at) WHERE archived_at IS NOT NULL;
