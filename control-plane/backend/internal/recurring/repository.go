...
import (
	...
	"github.com/devpablocristo/pymes/control-plane/backend/internal/recurring/repository/models"
...
func (r *Repository) List(ctx context.Context, orgID uuid.UUID, activeOnly bool, limit int) ([]recurringdomain.RecurringExpense, error) {
	...
	for _, row := range rows { out = append(out, toDomain(row)) }
	return out, nil
}
...
func (r *Repository) Create(ctx context.Context, in recurringdomain.RecurringExpense) (recurringdomain.RecurringExpense, error) {
	...
	if err := r.db.WithContext(ctx).Create(&row).Error; err != nil { return recurringdomain.RecurringExpense{}, err }
...
func (r *Repository) GetByID(ctx context.Context, orgID, id uuid.UUID) (recurringdomain.RecurringExpense, error) {
	...
	if err := r.db.WithContext(ctx).Where("org_id = ? AND id = ?", orgID, id).Take(&row).Error; err != nil { return recurringdomain.RecurringExpense{}, err }
	return toDomain(row), nil
}
...
func (r *Repository) Update(ctx context.Context, in recurringdomain.RecurringExpense) (recurringdomain.RecurringExpense, error) {
	...
	if err := r.db.WithContext(ctx).Save(&row).Error; err != nil { return recurringdomain.RecurringExpense{}, err }
	return toDomain(row), nil
}
...
func (r *Repository) GetCurrency(ctx context.Context, orgID uuid.UUID) string {
	var cur string
	if err := r.db.WithContext(ctx).Table("tenant_settings").Select("currency").Where("org_id = ?", orgID).Take(&cur).Error; err != nil || cur == "" { return "ARS" }
	return cur
}
