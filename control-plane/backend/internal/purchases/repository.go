...
type tenantSettings struct { PurchasePrefix string `gorm:"column:purchase_prefix"`; NextPurchaseNumber int `gorm:"column:next_purchase_number"`; Currency string `gorm:"column:currency"`; TaxRate float64 `gorm:"column:tax_rate"` }
...
func (r *Repository) List(ctx context.Context, orgID uuid.UUID, status string, limit int) ([]purchasesdomain.Purchase, error) {
	...
	if err := q.Order("created_at DESC").Limit(limit).Find(&rows).Error; err != nil { return nil, err }
	out := make([]purchasesdomain.Purchase, 0, len(rows))
	for _, row := range rows { out = append(out, toDomain(row, nil)) }
	return out, nil
}
...
func (r *Repository) Create(ctx context.Context, in CreateInput) (purchasesdomain.Purchase, error) {
	...
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Create(&purchaseRow).Error; err != nil { return err }
		items := make([]models.PurchaseItemModel, 0, len(in.Items))
		for _, item := range in.Items {
			pid := item.ProductID
			id := item.ID
			if id == uuid.Nil { id = uuid.New() }
		...
		if len(items) > 0 { if err := tx.Create(&items).Error; err != nil { return err } }
		if err := tx.Table("tenant_settings").Where("org_id = ?", in.OrgID).Updates(map[string]any{"next_purchase_number": tenant.NextPurchaseNumber + 1, "updated_at": gorm.Expr("now()")}).Error; err != nil { return err }
		if in.Status == "received" { if err := r.applyStock(ctx, tx, in.OrgID, purchaseRow.ID, items, in.CreatedBy); err != nil { return err } }
		out = toDomain(purchaseRow, items)
		return nil
	})
	if err != nil { return purchasesdomain.Purchase{}, err }
	return out, nil
}
...
func (r *Repository) GetByID(ctx context.Context, orgID, id uuid.UUID) (purchasesdomain.Purchase, error) {
	...
	if err := r.db.WithContext(ctx).Where("org_id = ? AND id = ?", orgID, id).Take(&row).Error; err != nil { return purchasesdomain.Purchase{}, err }
	var items []models.PurchaseItemModel
	if err := r.db.WithContext(ctx).Where("purchase_id = ?", id).Order("sort_order ASC").Find(&items).Error; err != nil { return purchasesdomain.Purchase{}, err }
...
func (r *Repository) Update(ctx context.Context, in UpdateInput) (purchasesdomain.Purchase, error) {
	...
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("org_id = ? AND id = ?", in.OrgID, in.ID).Take(&current).Error; err != nil { return err }
		subtotal, taxTotal, total := totals(in.Items)
		updates := map[string]any{"supplier_id": in.SupplierID, "supplier_name": in.SupplierName, "status": in.Status, "payment_status": in.PaymentStatus, "subtotal": subtotal, "tax_total": taxTotal, "total": total, "notes": in.Notes, "updated_at": time.Now().UTC()}
		if in.Status == "received" && current.ReceivedAt == nil { updates["received_at"] = time.Now().UTC() }
		if err := tx.Model(&models.PurchaseModel{}).Where("org_id = ? AND id = ?", in.OrgID, in.ID).Updates(updates).Error; err != nil { return err }
		if err := tx.Where("purchase_id = ?", in.ID).Delete(&models.PurchaseItemModel{}).Error; err != nil { return err }
		items := make([]models.PurchaseItemModel, 0, len(in.Items))
		for _, item := range in.Items { id := item.ID; if id == uuid.Nil { id = uuid.New() }; items = append(items, models.PurchaseItemModel{ID: id, PurchaseID: in.ID, ProductID: item.ProductID, Description: item.Description, Quantity: item.Quantity, UnitCost: item.UnitCost, TaxRate: item.TaxRate, Subtotal: item.Subtotal, SortOrder: item.SortOrder}) }
		if len(items) > 0 { if err := tx.Create(&items).Error; err != nil { return err } }
		if current.Status != "received" && in.Status == "received" { if err := r.applyStock(ctx, tx, in.OrgID, in.ID, items, current.CreatedBy); err != nil { return err } }
...
func (r *Repository) GetCurrency(ctx context.Context, orgID uuid.UUID) string { tenant, err := r.loadTenant(ctx, r.db, orgID); if err != nil { return "ARS" }; return tenant.Currency }
func (r *Repository) GetTaxRate(ctx context.Context, orgID uuid.UUID) float64 { tenant, err := r.loadTenant(ctx, r.db, orgID); if err != nil { return 21 }; return tenant.TaxRate }
...
func (r *Repository) loadTenant(ctx context.Context, db *gorm.DB, orgID uuid.UUID) (tenantSettings, error) {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) { return tenantSettings{PurchasePrefix: "CPA", NextPurchaseNumber: 1, Currency: "ARS", TaxRate: 21}, nil }
		return tenantSettings{}, err
	}
	if strings.TrimSpace(row.PurchasePrefix) == "" { row.PurchasePrefix = "CPA" }
	if row.NextPurchaseNumber <= 0 { row.NextPurchaseNumber = 1 }
	if strings.TrimSpace(row.Currency) == "" { row.Currency = "ARS" }
	if row.TaxRate <= 0 { row.TaxRate = 21 }
	return row, nil
}
...
func (r *Repository) getOrCreateTenantSettingsForUpdate(ctx context.Context, tx *gorm.DB, orgID uuid.UUID) (tenantSettings, error) {
	...
	if err == nil { return row, nil }
	if err := tx.WithContext(ctx).Exec(`INSERT INTO tenant_settings (org_id, plan_code, hard_limits, currency, tax_rate, purchase_prefix, next_purchase_number, created_at, updated_at) VALUES (?, 'starter', '{}'::jsonb, 'ARS', 21.0, 'CPA', 1, now(), now()) ON CONFLICT (org_id) DO NOTHING`, orgID).Error; err != nil { return tenantSettings{}, err }
...
func (r *Repository) applyStock(ctx context.Context, tx *gorm.DB, orgID, purchaseID uuid.UUID, items []models.PurchaseItemModel, actor string) error {
	for _, item := range items {
		if item.ProductID == nil || *item.ProductID == uuid.Nil || item.Quantity <= 0 { continue }
		if err := tx.Exec(`
		...
		`, *item.ProductID, orgID, item.Quantity).Error; err != nil { return err }
		if err := tx.Exec(`
		...
		`, orgID, *item.ProductID, item.Quantity, purchaseID, item.Description, actor).Error; err != nil { return err }
	}
	return nil
}
...
func totals(items []purchasesdomain.PurchaseItem) (float64, float64, float64) {
	subtotal := 0.0
	taxTotal := 0.0
	for _, item := range items { line := item.Quantity * item.UnitCost; subtotal += line; taxTotal += line * item.TaxRate / 100 }
...
func toDomain(row models.PurchaseModel, items []models.PurchaseItemModel) purchasesdomain.Purchase {
	...
	for _, item := range items { out.Items = append(out.Items, purchasesdomain.PurchaseItem{ID: item.ID, PurchaseID: item.PurchaseID, ProductID: item.ProductID, Description: item.Description, Quantity: item.Quantity, UnitCost: item.UnitCost, TaxRate: item.TaxRate, Subtotal: item.Subtotal, SortOrder: item.SortOrder}) }
	return out
}
