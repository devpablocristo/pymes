...
from src.api.professionals_deps import get_auth_context, get_professionals_backend_client, get_llm_provider
...
from src.core.professionals_system_prompt import build_system_prompt
...
from src.tools.professionals.registry import build_internal_tools

router = APIRouter(prefix="/v1/professionals/chat", tags=["professionals-chat"])
...
@router.post("")
async def chat_professionals(
    req: ChatRequest,
    auth: AuthContext = Depends(get_auth_context),
    llm=Depends(get_llm_provider),
    backend_client: ProfessionalsBackendClient = Depends(get_professionals_backend_client),
):
    logger.info("professionals_chat_started", org_id=auth.org_id, user_id=auth.actor)
    ...
    async def event_stream():
        ...
        try:
            ...
            logger.exception("professionals_chat_failed", org_id=auth.org_id, user_id=auth.actor, error=str(exc))
