...
import { createInsightNotifications } from '../lib/aiApi';
...
export function NotificationsCenterPage({ embedded = false }: NotificationsCenterPageProps) {
  ...
  const inAppError = notificationsQuery.error
    ...
    : '';
  const generationError = generateInsightsMutation.error
    ? formatFetchErrorForUser(generateInsightsMutation.error, t('ai.chat.error.unreachable'))
    : '';
  ...
  useEffect(() => {
    ...
    if (generateInsightsMutation.isPending) return;
    autoGeneratedRef.current = true;
    void generateInsightsMutation.mutateAsync();
  }, [generateInsightsMutation, notifications.length, notificationsQuery.isSuccess]);
  ...
  const feed = (
    <>
      <div className="notifications-toolbar">
        <div className="notifications-toolbar__hint">Insights semanales para llenar la bandeja con contexto útil.</div>
        <button
          type="button"
          className="btn-secondary btn-sm"
          disabled={generateInsightsMutation.isPending}
          onClick={() => void generateInsightsMutation.mutateAsync()}
        >
          {generateInsightsMutation.isPending ? 'Generando insights…' : 'Generar insights'}
        </button>
      </div>
      <NotificationFeed
        className="notifications-feed"
        error={
          inAppError || generationError ? (
            <p role="alert" className="form-error">
              {inAppError || generationError}
            </p>
          ) : undefined
        }
        loading={notificationsQuery.isLoading || generateInsightsMutation.isPending}
