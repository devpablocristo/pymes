...
func (u *Usecases) Update(ctx context.Context, orgID, id uuid.UUID, in UpdateInput, actor string) (domain.Bicycle, error) {
	...
	if in.FrameNumber != nil {
		current.FrameNumber = normalizeFrameNumber(*in.FrameNumber)
	}
	if in.Brand != nil {
...
func (u *Usecases) HardDelete(ctx context.Context, orgID, id uuid.UUID, actor string) error {
	if err := u.repo.HardDelete(ctx, orgID, id); err != nil {
		...
		if errors.Is(err, ErrBicycleHasWorkOrders) {
			return fmt.Errorf("bicycle has work orders: %w", httperrors.ErrConflict)
		}
		return err
	}
	if u.audit != nil {
...
func validateBicycle(in *domain.Bicycle) error {
	...
	in.FrameNumber = normalizeFrameNumber(in.FrameNumber)
	...
	if len(in.Brand) < 2 {
		return fmt.Errorf("brand is required: %w", httperrors.ErrBadInput)
	}
	if len(in.Model) < 1 {
		return fmt.Errorf("model is required: %w", httperrors.ErrBadInput)
	}
	if in.WheelSizeInches < 0 || in.WheelSizeInches > 40 {
		return fmt.Errorf("wheel_size_inches is invalid: %w", httperrors.ErrBadInput)
	}
	return nil
}

func normalizeFrameNumber(raw string) string {
