...
def build_internal_tools(
    client: ProfessionalsBackendClient,
    auth: AuthContext,
    ...
    async def _get_professional_profiles(org_id: str) -> dict[str, Any]:
        _ = org_id
        return await client.get_professionals(auth)

    declarations.append(
        _tool(
            "get_professional_profiles",
            "Listar perfiles de profesionales del estudio",
            {"type": "object", "properties": {}},
        )
    )
    handlers["get_professional_profiles"] = _get_professional_profiles
    ...
    async def _get_professional_catalog(org_id: str, profile_id: str) -> dict[str, Any]:
        _ = org_id
        return await client.get_profile_services(auth, profile_id=profile_id)
    ...
    declarations.append(
        _tool(
            ...
            "Listar servicios de un profesional",
            {
                "type": "object",
                "properties": {
                    "profile_id": {"type": "string", "description": "UUID del profesional"},
                },
                "required": ["profile_id"],
            },
        )
    )
    handlers["get_professional_catalog"] = _get_professional_catalog
    ...
    async def _get_today_schedule(org_id: str, date: str = "today") -> dict[str, Any]:
        _ = org_id
        filters = {"date": date}
        return await client.get_sessions(auth, filters=filters)
    ...
    async def _create_intake(
        ...
        return await client.create_intake(auth, data={k: v for k, v in data.items() if v})

    declarations.append(
        _tool(
            "create_intake",
            "Crear ficha de intake para un nuevo paciente/cliente",
            {
                "type": "object",
                "properties": {
                    "client_name": {"type": "string", "description": "Nombre del paciente/cliente"},
                    "client_phone": {"type": "string", "description": "Telefono"},
    ...
    async def _prepare_quote(
        ...
        return await client.prepare_quote(auth, data=data)
            ...
            "Preparar presupuesto para un cliente",
            {
                ...
                "properties": {
                    ...
                    "items": {
                        "type": "array",
                        "items": {"type": "object"},
                        "description": "Lista de servicios con nombre, precio, cantidad",
                    },
                    "notes": {"type": "string"},
                },
    ...
    declarations.append(
        _tool(
            "get_payment_link",
            "Generar link de pago para una venta o presupuesto",
            {
                "type": "object",
                "properties": {
                    "sale_id": {"type": "string", "description": "UUID de la venta o presupuesto"},
                },
                "required": ["sale_id"],
            },
        )
...
def build_external_tools(
    ...
    async def _get_public_professionals(org_id: str) -> dict[str, Any]:
        _ = org_id
        return await client.get_public_professionals(org_slug)

    declarations.append(
        _tool(
            "get_public_professionals",
            "Listar profesionales del estudio con sus especialidades",
            {"type": "object", "properties": {}},
        )
    )
    handlers["get_public_professionals"] = _get_public_professionals
