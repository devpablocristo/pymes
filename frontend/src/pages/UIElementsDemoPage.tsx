/**
 * UI Elements demo — showcase de 19 categorías de componentes UI.
...
function AlertsSection() {
  return (
    <div className="card">
      <h3 className="ui-demo__section-title">Alertas</h3>
      <div className="ui-demo__alert ui-demo__alert--info">Información — Este es un mensaje informativo.</div>
      <div className="ui-demo__alert ui-demo__alert--success">Éxito — La operación se completó correctamente.</div>
      <div className="ui-demo__alert ui-demo__alert--warning">Advertencia — Revisá los datos antes de continuar.</div>
      <div className="ui-demo__alert ui-demo__alert--danger">Error — No se pudo completar la acción.</div>
    </div>
  );
}
...
function AvatarsSection() {
  ...
  return (
    <div className="card">
      <h3 className="ui-demo__section-title">Avatares — tamaños</h3>
      <div className="ui-demo__row">
        {sizes.map((s) => (
          <div key={s.cls} className={`ui-demo__avatar ui-demo__avatar--${s.cls}`} style={{ background: '#3b82f6' }}>{s.label}</div>
        ))}
      </div>
      <h3 className="ui-demo__section-title" style={{ marginTop: 'var(--space-4)' }}>Avatares — colores</h3>
      <div className="ui-demo__row">
        {COLORS.slice(0, 6).map((c) => (
          <div key={c.name} className="ui-demo__avatar ui-demo__avatar--md" style={{ background: c.hex }}>{c.name.slice(0, 2)}</div>
        ))}
      </div>
    </div>
  );
...
function BadgesSection() {
  return (
    <div className="card">
      <h3 className="ui-demo__section-title">Badges</h3>
      <div className="ui-demo__row">
      ...
      <h3 className="ui-demo__section-title" style={{ marginTop: 'var(--space-4)' }}>Badges con contexto</h3>
      <div className="ui-demo__row">
...
function ButtonsSection() {
  return (
    <div className="card">
      <h3 className="ui-demo__section-title">Botones — variantes</h3>
      <div className="ui-demo__row">
      ...
      <h3 className="ui-demo__section-title" style={{ marginTop: 'var(--space-4)' }}>Tamaños</h3>
      <div className="ui-demo__row">
        <button type="button" className="btn-primary btn-sm">Pequeño</button>
        <button type="button" className="btn-primary">Normal</button>
      </div>
      <h3 className="ui-demo__section-title" style={{ marginTop: 'var(--space-4)' }}>Estados</h3>
      <div className="ui-demo__row">
...
function CardsSection() {
  return (
    <div className="ui-demo__grid" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))' }}>
      <div className="card"><h3 className="ui-demo__section-title">Card básica</h3><p style={{ fontSize: '0.85rem', color: 'var(--color-text-secondary)' }}>Contenido simple dentro de una card con padding estándar.</p></div>
      <div className="card" style={{ padding: 0 }}>
        <div style={{ height: 120, background: 'linear-gradient(135deg, #3b82f6, #8b5cf6)', borderRadius: 'var(--radius-lg) var(--radius-lg) 0 0' }} />
        <div style={{ padding: 'var(--space-4)' }}><h3 className="ui-demo__section-title">Card con imagen</h3><p style={{ fontSize: '0.85rem', color: 'var(--color-text-secondary)' }}>Header visual con contenido debajo.</p></div>
      </div>
...
function CarouselSection() {
  ...
  return (
    <div className="card">
      <h3 className="ui-demo__section-title">Carrusel</h3>
      <div style={{ borderRadius: 'var(--radius-md)', overflow: 'hidden', position: 'relative', height: 180, background: slides[idx], display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: '1.5rem', fontWeight: 700, transition: 'background 0.3s ease' }}>
        Slide {idx + 1}
      </div>
      <div className="ui-demo__row" style={{ justifyContent: 'center', marginTop: 'var(--space-3)' }}>
...
function ColorsSection() {
  return (
    <div className="card">
      <h3 className="ui-demo__section-title">Paleta de colores</h3>
      <div className="ui-demo__grid" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))' }}>
        {COLORS.map((c) => (
          <div key={c.name} className="ui-demo__swatch">
            <div className="ui-demo__swatch-color" style={{ background: c.hex }} />
            <div className="ui-demo__swatch-info"><strong>{c.name}</strong><br />{c.hex}</div>
          </div>
        ))}
      </div>
    </div>
...
function DropdownsSection() {
  ...
  return (
    <div className="card">
      <h3 className="ui-demo__section-title">Dropdowns</h3>
      <div className="ui-demo__row">
...
function PaginationSection() {
  ...
  return (
    <div className="card">
      <h3 className="ui-demo__section-title">Paginación</h3>
      <div className="ui-demo__row">
...
function ProgressSection() {
  ...
  return (
    <div className="card">
      <h3 className="ui-demo__section-title">Barras de progreso</h3>
      {bars.map((b) => (
        <div key={b.label}>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.78rem', marginBottom: '0.2rem' }}>
            <span>{b.label}</span><span>{b.pct}%</span>
          </div>
          <div className="ui-demo__progress"><div className="ui-demo__progress-fill" style={{ width: `${b.pct}%`, background: b.color }} /></div>
        </div>
      ))}
    </div>
  );
...
function RadioSection() {
  ...
  return (
    <div className="card">
      <h3 className="ui-demo__section-title">Radio buttons</h3>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
        {[{ id: 'a', label: 'Opción A' }, { id: 'b', label: 'Opción B' }, { id: 'c', label: 'Opción C (deshabilitada)' }].map((o) => (
          <label key={o.id} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.85rem', cursor: o.id === 'c' ? 'not-allowed' : 'pointer', opacity: o.id === 'c' ? 0.5 : 1 }}>
            <input type="radio" name="demo-radio" checked={v === o.id} onChange={() => setV(o.id)} disabled={o.id === 'c'} />
            {o.label}
          </label>
        ))}
      </div>
...
function SwitchesSection() {
  ...
  return (
    <div className="card">
      <h3 className="ui-demo__section-title">Switches</h3>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
        <label style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', fontSize: '0.85rem' }}>
          <label className="ui-demo__switch"><input type="checkbox" checked={s1} onChange={() => setS1(!s1)} /><span className="ui-demo__switch-track" /></label>
          Activo ({s1 ? 'ON' : 'OFF'})
        </label>
        <label style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', fontSize: '0.85rem' }}>
          <label className="ui-demo__switch"><input type="checkbox" checked={s2} onChange={() => setS2(!s2)} /><span className="ui-demo__switch-track" /></label>
...
function TabsSection() {
  ...
  return (
    <div className="card">
      <h3 className="ui-demo__section-title">Tabs</h3>
      <div className="ui-demo__demo-tabs">
        {['General', 'Perfil', 'Seguridad'].map((label, i) => (
          <button key={label} type="button" className={`ui-demo__demo-tab ${t === i ? 'ui-demo__demo-tab--active' : ''}`} onClick={() => setT(i)}>{label}</button>
        ))}
      </div>
...
function TagsSection() {
  ...
  return (
    <div className="card">
      <h3 className="ui-demo__section-title">Tags</h3>
      <div className="ui-demo__row">
...
function TooltipsSection() {
  return (
    <div className="card">
      <h3 className="ui-demo__section-title">Tooltips</h3>
      <div className="ui-demo__row">
...
function TypographySection() {
  return (
    <div className="card">
      <h3 className="ui-demo__section-title">Tipografía</h3>
      <h1 style={{ margin: '0 0 0.5rem' }}>Heading 1</h1>
...
function ListsSection() {
  ...
  return (
    <div className="card">
      <h3 className="ui-demo__section-title">Listas</h3>
...
function VideosSection() {
  return (
    <div className="card">
      <h3 className="ui-demo__section-title">Video embebido</h3>
      <div className="ui-demo__video-wrap">
        <iframe src="https://www.youtube.com/embed/dQw4w9WgXcQ" title="Video demo" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen />
      </div>
    </div>
  );
}
...
export function UIElementsDemoPage() {
    ...
    <div className="ui-demo">
      <div className="page-header">
        ...
        <p style={{ color: 'var(--color-text-secondary)', margin: 0, fontSize: '0.88rem' }}>
          Showcase de elementos de interfaz — alertas, botones, cards, formularios y más
        </p>
      </div>

      <div className="ui-demo__tabs">
        {TABS.map((t) => (
          <button key={t.id} type="button" className={`ui-demo__tab ${tab === t.id ? 'ui-demo__tab--active' : ''}`} onClick={() => setTab(t.id)}>
            {t.label}
          </button>
        ))}
      </div>
...
export default UIElementsDemoPage;
