...
class AutoRepairBackendClient(HTTPBackendClient):
    ...
    async def list_work_orders(self, auth: AuthContext, status: str = "", search: str = "") -> dict[str, Any]:
        params: dict[str, str] = {"asset_type": "vehicle"}
        if status:
            params["status"] = status
        if search:
            params["search"] = search
        return await self.request("GET", "/v1/auto-repair/work-orders", auth=auth, params=params)
    ...
    async def create_sale(self, auth: AuthContext, work_order_id: str) -> dict[str, Any]:
        return await self.request("POST", f"/v1/auto-repair/work-orders/{work_order_id}/sale", auth=auth)
    ...
    async def public_book_scheduling(self, tenant_slug: str, data: dict[str, Any]) -> dict[str, Any]:
        return await self.request("POST", f"/v1/public/{tenant_slug}/auto-repair/bookings", include_internal=True, json=data)
