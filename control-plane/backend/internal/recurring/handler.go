...
func (h *Handler) Delete(c *gin.Context) {
	...
	orgID, id, ok := parseOrgID(c)
	if !ok { return }
	if err := h.uc.Deactivate(c.Request.Context(), orgID, id, authCtx.Actor); err != nil { httperrors.Respond(c, err); return }
...
func createRecurringPayload(orgID uuid.UUID, req dto.CreateRecurringExpenseRequest, actor string) (recurringdomain.RecurringExpense, error) {
	...
	if req.SupplierID != nil && strings.TrimSpace(*req.SupplierID) != "" { parsed, err := uuid.Parse(strings.TrimSpace(*req.SupplierID)); if err != nil { return recurringdomain.RecurringExpense{}, httperrors.ErrBadInput }; supplierID = &parsed }
	nextDueDate := time.Time{}
	if strings.TrimSpace(req.NextDueDate) != "" { parsed, err := time.Parse("2006-01-02", strings.TrimSpace(req.NextDueDate)); if err != nil { return recurringdomain.RecurringExpense{}, httperrors.ErrBadInput }; nextDueDate = parsed.UTC() }
...
func updateRecurringPayload(orgID, id uuid.UUID, req dto.UpdateRecurringExpenseRequest) (recurringdomain.RecurringExpense, error) {
	...
	if req.Description != nil { payload.Description = strings.TrimSpace(*req.Description) }
	if req.Amount != nil { payload.Amount = *req.Amount }
	if req.Currency != nil { payload.Currency = strings.TrimSpace(*req.Currency) }
	if req.Category != nil { payload.Category = strings.TrimSpace(*req.Category) }
	if req.PaymentMethod != nil { payload.PaymentMethod = strings.TrimSpace(*req.PaymentMethod) }
	if req.Frequency != nil { payload.Frequency = strings.TrimSpace(*req.Frequency) }
	if req.DayOfMonth != nil { payload.DayOfMonth = *req.DayOfMonth }
	if req.SupplierID != nil && strings.TrimSpace(*req.SupplierID) != "" { parsed, err := uuid.Parse(strings.TrimSpace(*req.SupplierID)); if err != nil { return recurringdomain.RecurringExpense{}, httperrors.ErrBadInput }; payload.SupplierID = &parsed }
	if req.IsActive != nil { payload.IsActive = *req.IsActive }
	if req.NextDueDate != nil && strings.TrimSpace(*req.NextDueDate) != "" { parsed, err := time.Parse("2006-01-02", strings.TrimSpace(*req.NextDueDate)); if err != nil { return recurringdomain.RecurringExpense{}, httperrors.ErrBadInput }; payload.NextDueDate = parsed.UTC() }
	if req.Notes != nil { payload.Notes = strings.TrimSpace(*req.Notes) }
	return payload, nil
}
