...
import (
	...
	"github.com/devpablocristo/pymes/pymes-core/backend/internal/whatsapp/handler/dto"
...
func (h *Handler) RegisterRoutes(auth *gin.RouterGroup, rbac *handlers.RBACMiddleware) {
	...
	{
		// Connection
		wa.GET("/connection", h.GetConnection)
		wa.POST("/connection", h.Connect)
		wa.DELETE("/connection", h.Disconnect)
		wa.GET("/connection/stats", h.GetConnectionStats)

		// Sending
		wa.POST("/send/text", rbac.RequirePermission("whatsapp", "write"), h.SendText)
		wa.POST("/send/template", rbac.RequirePermission("whatsapp", "write"), h.SendTemplate)
		wa.POST("/send/media", rbac.RequirePermission("whatsapp", "write"), h.SendMedia)
		wa.POST("/send/interactive", rbac.RequirePermission("whatsapp", "write"), h.SendInteractive)

		// Messages
		wa.GET("/messages", rbac.RequirePermission("whatsapp", "read"), h.ListMessages)

		// Templates
		wa.GET("/templates", rbac.RequirePermission("whatsapp", "read"), h.ListTemplates)
		wa.POST("/templates", rbac.RequirePermission("whatsapp", "write"), h.CreateTemplate)
		wa.GET("/templates/:id", rbac.RequirePermission("whatsapp", "read"), h.GetTemplate)
		wa.DELETE("/templates/:id", rbac.RequirePermission("whatsapp", "write"), h.DeleteTemplate)

		// Opt-in
		wa.GET("/opt-ins", rbac.RequirePermission("whatsapp", "read"), h.ListOptIns)
...
func (h *Handler) GetConnection(c *gin.Context) {
	...
	c.JSON(http.StatusOK, dto.ConnectionResponse{
		OrgID:              conn.OrgID,
...
func (h *Handler) Connect(c *gin.Context) {
	...
	c.JSON(http.StatusCreated, dto.ConnectionResponse{
		OrgID:              conn.OrgID,
		PhoneNumberID:      conn.PhoneNumberID,
		WABAID:             conn.WABAID,
		DisplayPhoneNumber: conn.DisplayPhoneNumber,
...
func (h *Handler) GetConnectionStats(c *gin.Context) {
	...
	c.JSON(http.StatusOK, dto.ConnectionStatsResponse{
...
func (h *Handler) SendText(c *gin.Context) {
	...
	msg, err := h.uc.SendText(c.Request.Context(), domain.SendTextRequest{
		OrgID:   orgID,
		PartyID: partyID,
		Body:    body.Body,
		Actor:   auth.Actor,
...
func (h *Handler) SendTemplate(c *gin.Context) {
	...
	msg, err := h.uc.SendTemplate(c.Request.Context(), domain.SendTemplateRequest{
		OrgID:        orgID,
		PartyID:      partyID,
...
func (h *Handler) SendMedia(c *gin.Context) {
	...
	msg, err := h.uc.SendMedia(c.Request.Context(), domain.SendMediaRequest{
		OrgID:     orgID,
		PartyID:   partyID,
		MediaType: domain.MessageType(body.MediaType),
...
func (h *Handler) SendInteractive(c *gin.Context) {
	...
	msg, err := h.uc.SendInteractive(c.Request.Context(), domain.SendInteractiveRequest{
		OrgID:   orgID,
		PartyID: partyID,
		Body:    body.Body,
		Buttons: buttons,
...
func (h *Handler) CreateTemplate(c *gin.Context) {
	...
	tpl, err := h.uc.CreateTemplate(c.Request.Context(), orgID, domain.Template{
		Name:       body.Name,
		Language:   body.Language,
		Category:   domain.TemplateCategory(body.Category),
